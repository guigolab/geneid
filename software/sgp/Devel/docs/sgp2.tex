...
%
% $Id: sgp2.tex,v 1.10 2000-10-19 11:31:29 jabril Exp $
%  
...
\begin{center}
{\Large\sgp}\\[1ex]
{\large\verb+$Id: sgp2.tex,v 1.10 2000-10-19 11:31:29 jabril Exp $+}\\[2ex]
\end{center}
...
{\Tt{}\LA{}declare variables~{\nwtagstyle{}\subpageref{NWsgp9-decH-1}}\RA{}} will collect all the variables that need to be declared global within this program.
...
'\cd{perl -w}' prints all sorts of useful and interesting warning messages at compile time, but comment {\Tt{}-w} for stable versions. 
...
my $PROGRAM = "sgp2";
my @tmp_ver = split / +/, ' $Id: sgp2.tex,v 1.10 2000-10-19 11:31:29 jabril Exp $ ';
...
\nwalsodefined{\\{NWsgp9-defF-2}\\{NWsgp9-defF-3}}\nwused{\\{NWsgp9-sgp7-1}}\nwidentdefs{\\{{\nwixident{{\$}SCF}}{:doSCF}}\\{{\nwixident{{\$}S{\_}CUTOFF}}{:doS:unCUTOFF}}}\nwendcode{}\nwbegindocs{12}\nwdocspar
...
{\Tt{}{\$}SGPTMP} %$ 
contains the prefix for temporal files that is set different by adding the process ID ({\Tt{}{\$}{\$}} in plain {\Tt{}perl}, if ``{\Tt{}use\ English;}'' is given it can also be retrieved as {\Tt{}{\$}PROCESS{\_}ID} or {\Tt{}{\$}PID}).

\todo{define defaults}{
\item Check whether environmental path variables ({\Tt{}{\$}ENV{\nwlbrace}'SGP'{\nwrbrace}} and {\Tt{}{\$}ENV{\nwlbrace}'SGPTMP'{\nwrbrace}}) settings does exist.
...
\nwenddocs{}\nwbegincode{37}\sublabel{NWsgp9-defF-3}\nwmargintag{{\nwtagstyle{}\subpageref{NWsgp9-defF-3}}}\moddef{define defaults~{\nwtagstyle{}\subpageref{NWsgp9-defF-1}}}\plusendmoddef\nwstartdeflinemarkup\nwusesondefline{\\{NWsgp9-sgp7-1}}\nwprevnextdefs{NWsgp9-defF-2}{\relax}\nwenddeflinemarkup
my $verbose_str = " -v ";
\nwindexdefn{\nwixident{{\$}verbose{\_}str}}{:doverbose:unstr}{NWsgp9-defF-3}\eatline
\nwused{\\{NWsgp9-sgp7-1}}\nwidentdefs{\\{{\nwixident{{\$}verbose{\_}str}}{:doverbose:unstr}}}\nwendcode{}\nwbegincode{38}\sublabel{NWsgp9-setE-2}\nwmargintag{{\nwtagstyle{}\subpageref{NWsgp9-setE-2}}}\moddef{set flags-vars~{\nwtagstyle{}\subpageref{NWsgp9-setE-1}}}\plusendmoddef\nwstartdeflinemarkup\nwusesondefline{\\{NWsgp9-getB-1}}\nwprevnextdefs{NWsgp9-setE-1}{\relax}\nwenddeflinemarkup
do \{
...
{\Tt{}prt{\_}progress(counter)} and {\Tt{}prt{\_}foeprg(counter)} are used for progress report on STDERR, they both need a \cv{counter} integer that is used to calculate whether a newline (including a subtotal in the former, or the total for the last) or a progress item ('.') is sent to STDERR.

{\Tt{}fill{\_}right(string,max{\_}length,fill{\_}char)}, {\Tt{}fill{\_}left(string,max{\_}length,fill{\_}char)} and {\Tt{}fill{\_}mid(string,max{\_}length,fill{\_}char)} will reformat any given \cv{string} by adding \cv{fill\_char} (blanks, dots, \ldots) to the left, right or both ends respectively, obtaining a final string with the given \cv{max\_length}. {\Tt{}prt{\_}Header(string)} surrounds \cv{string} with a '*' frame, reporting to STDERR that we are executing a new program section.

{\Tt{}max(list)} returns the maximum value from within the elements of \cv{list}.

\nwenddocs{}\nwbegincode{48}\sublabel{NWsgp9-gloI-5}\nwmargintag{{\nwtagstyle{}\subpageref{NWsgp9-gloI-5}}}\moddef{global subroutines~{\nwtagstyle{}\subpageref{NWsgp9-gloI-1}}}\plusendmoddef\nwstartdeflinemarkup\nwusesondefline{\\{NWsgp9-sgp7-1}}\nwprevnextdefs{NWsgp9-gloI-4}{\relax}\nwenddeflinemarkup
# Timing.
sub get_exec_time() \{
    $verbose_flg && do \{
        my $End = $_[0];
...
{\Tt{}get{\_}exec{\_}time(time)} returns execution time since {\Tt{}{\$}Start}.%$ 
The best choice taken until now is to provide as input the built-in perl function {\Tt{}time}. Maybe a better timming procedure will be implemented in a future if needed.
 
\subsctn{Running {\Tt{}blast} on sequences}

\nwenddocs{}\nwbegincode{50}\sublabel{NWsgp9-decH-4}\nwmargintag{{\nwtagstyle{}\subpageref{NWsgp9-decH-4}}}\moddef{declare variables~{\nwtagstyle{}\subpageref{NWsgp9-decH-1}}}\plusendmoddef\nwstartdeflinemarkup\nwusesondefline{\\{NWsgp9-sgp7-1}}\nwprevnextdefs{NWsgp9-decH-3}{NWsgp9-decH-5}\nwenddeflinemarkup
# Processes
my $status;
# Blast 
...
\nwenddocs{}\nwbegincode{54}\sublabel{NWsgp9-decH-5}\nwmargintag{{\nwtagstyle{}\subpageref{NWsgp9-decH-5}}}\moddef{declare variables~{\nwtagstyle{}\subpageref{NWsgp9-decH-1}}}\plusendmoddef\nwstartdeflinemarkup\nwusesondefline{\\{NWsgp9-sgp7-1}}\nwprevnextdefs{NWsgp9-decH-4}{NWsgp9-decH-6}\nwenddeflinemarkup
# Extract HSPs
my $GetSRsProgram = "$SGP2/GetSRsAln.pl";
\nwused{\\{NWsgp9-sgp7-1}}\nwidentuses{\\{{\nwixident{{\$}SGP2}}{:doSGP2}}}\nwindexuse{\nwixident{{\$}SGP2}}{:doSGP2}{NWsgp9-decH-5}\nwendcode{}\nwbegindocs{55} %def $GetSRsProgram

Take care that {\Tt{}GetSRsAln} program requires {\Tt{}parseblast} filter. We are using the following {\Tt{}GetSRsAln} options:
\begin{itemize}\setlength{\itemsep}{-0.5ex}
...
\item check if paths to {\Tt{}parseblast} are correctly defined in {\Tt{}GetSRsAln}. Better if there is any way to define external program paths in a {\Tt{}Install} script.
\item In a future will be faster to implement most routines of {\Tt{}parseblast} and {\Tt{}GetSRsAln} in {\sgp}. This will increase speed by improving internal usage of the parsed data structures and by requiring less temporary files.
}

\nwenddocs{}\nwbegincode{56}\sublabel{NWsgp9-extC-1}\nwmargintag{{\nwtagstyle{}\subpageref{NWsgp9-extC-1}}}\moddef{extract HSPs~{\nwtagstyle{}\subpageref{NWsgp9-extC-1}}}\endmoddef\nwstartdeflinemarkup\nwusesondefline{\\{NWsgp9-sgp7-1}}\nwenddeflinemarkup
...
{\Tt{}write{\_}HSP{\_}files(input{\_}file,query{\_}file[,subject{\_}file[,align{\_}file})]] can receive a list, using the first element as input file, and the others (from one up to three) as output files. First output file ({\Tt{}query{\_}file}) is a GFF file built from GFF fields from {\Tt{}{\$}0} to {\Tt{}{\$}7}, second one ({\Tt{}subject{\_}file}) is built from GFF fields from {\Tt{}{\$}9} to {\Tt{}{\$}17}, finally, last file ({\Tt{}align{\_}file}) generates {\Tt{}gff2aplot} suitable records.
...
\nwused{\\{NWsgp9-sgp7-1}}\nwidentdefs{\\{{\nwixident{{\$}gff2ps}}{:dogff2ps}}\\{{\nwixident{{\&}Make{\_}Plots}}{:amMake:unPlots}}}\nwidentuses{\\{{\nwixident{{\$}aplot}}{:doaplot}}\\{{\nwixident{{\$}Gff2psOptions}}{:doGff2psOptions}}\\{{\nwixident{{\$}Gff2psParam}}{:doGff2psParam}}\\{{\nwixident{{\$}Gff2psProgram}}{:doGff2psProgram}}\\{{\nwixident{{\&}go{\_}to{\_}die}}{:amgo:unto:undie}}\\{{\nwixident{{\$}Loc1}}{:doLoc1}}\\{{\nwixident{{\$}Loc2}}{:doLoc2}}\\{{\nwixident{{\$}ps{\_}output}}{:dops:unoutput}}\\{{\nwixident{{\$}SGPtmp1}}{:doSGPtmp1}}\\{{\nwixident{{\$}SGPtmp2}}{:doSGPtmp2}}\\{{\nwixident{{\$}status}}{:dostatus}}\\{{\nwixident{{\$}verbose{\_}flg}}{:doverbose:unflg}}\\{{\nwixident{{\$}verbose{\_}str}}{:doverbose:unstr}}}\nwindexuse{\nwixident{{\$}aplot}}{:doaplot}{NWsgp9-graG-1}\nwindexuse{\nwixident{{\$}Gff2psOptions}}{:doGff2psOptions}{NWsgp9-graG-1}\nwindexuse{\nwixident{{\$}Gff2psParam}}{:doGff2psParam}{NWsgp9-graG-1}\nwindexuse{\nwixident{{\$}Gff2psProgram}}{:doGff2psProgram}{NWsgp9-graG-1}\nwindexuse{\nwixident{{\&}go{\_}to{\_}die}}{:amgo:unto:undie}{NWsgp9-graG-1}\nwindexuse{\nwixident{{\$}Loc1}}{:doLoc1}{NWsgp9-graG-1}\nwindexuse{\nwixident{{\$}Loc2}}{:doLoc2}{NWsgp9-graG-1}\nwindexuse{\nwixident{{\$}ps{\_}output}}{:dops:unoutput}{NWsgp9-graG-1}\nwindexuse{\nwixident{{\$}SGPtmp1}}{:doSGPtmp1}{NWsgp9-graG-1}\nwindexuse{\nwixident{{\$}SGPtmp2}}{:doSGPtmp2}{NWsgp9-graG-1}\nwindexuse{\nwixident{{\$}status}}{:dostatus}{NWsgp9-graG-1}\nwindexuse{\nwixident{{\$}verbose{\_}flg}}{:doverbose:unflg}{NWsgp9-graG-1}\nwindexuse{\nwixident{{\$}verbose{\_}str}}{:doverbose:unstr}{NWsgp9-graG-1}\nwendcode{}\nwbegindocs{66}\nwdocspar
\label{status256}{\Tt{}{\$}status} %$
is compared to 256 because this is the returned value by {\Tt{}system} from {\Tt{}gff2ps} shell script call when {\Tt{}exit(0)}, normal program exit, is returned by that shell. I still do not know why {\Tt{}system()} returns that value instead of 0 (as it happens with {\Tt{}blast}, {\Tt{}GetSRsAln} and {\Tt{}geneid} system calls). 

\nwenddocs{}\nwbegincode{67}\sublabel{NWsgp9-decH-8}\nwmargintag{{\nwtagstyle{}\subpageref{NWsgp9-decH-8}}}\moddef{declare variables~{\nwtagstyle{}\subpageref{NWsgp9-decH-1}}}\plusendmoddef\nwstartdeflinemarkup\nwusesondefline{\\{NWsgp9-sgp7-1}}\nwprevnextdefs{NWsgp9-decH-7}{\relax}\nwenddeflinemarkup
# aplot
my $AplotProgram  = "$SGP2/gff2aplot";
my $AplotParam    = "-O $SGP2param/sgp2-aplot.rc";
my $AplotOptions  = "";
\nwindexdefn{\nwixident{{\$}AplotProgram}}{:doAplotProgram}{NWsgp9-decH-8}\nwindexdefn{\nwixident{{\$}AplotParam}}{:doAplotParam}{NWsgp9-decH-8}\nwindexdefn{\nwixident{{\$}AplotOptions}}{:doAplotOptions}{NWsgp9-decH-8}\eatline
\nwused{\\{NWsgp9-sgp7-1}}\nwidentdefs{\\{{\nwixident{{\$}AplotOptions}}{:doAplotOptions}}\\{{\nwixident{{\$}AplotParam}}{:doAplotParam}}\\{{\nwixident{{\$}AplotProgram}}{:doAplotProgram}}}\nwidentuses{\\{{\nwixident{{\$}SGP2}}{:doSGP2}}\\{{\nwixident{{\$}SGP2param}}{:doSGP2param}}}\nwindexuse{\nwixident{{\$}SGP2}}{:doSGP2}{NWsgp9-decH-8}\nwindexuse{\nwixident{{\$}SGP2param}}{:doSGP2param}{NWsgp9-decH-8}\nwendcode{}\nwbegincode{68}\sublabel{NWsgp9-graE-1}\nwmargintag{{\nwtagstyle{}\subpageref{NWsgp9-graE-1}}}\moddef{graphical subs~{\nwtagstyle{}\subpageref{NWsgp9-graE-1}}}\endmoddef\nwstartdeflinemarkup\nwusesondefline{\\{NWsgp9-graG-1}}\nwenddeflinemarkup
sub load_gene_struc() \{
    my $PRT = shift @_;
    my $strand;
    open(GQ,"< $_[0]");
...
sub build_aplot_file() \{
  ...
  SAVEFILES: do \{
    ...
    \} if $savefiles_flg;
\}
\nwused{\\{NWsgp9-graG-1}}\nwidentuses{\\{{\nwixident{{\$}Loc2}}{:doLoc2}}\\{{\nwixident{{\$}ofn}}{:doofn}}\\{{\nwixident{{\$}savefiles{\_}flg}}{:dosavefiles:unflg}}\\{{\nwixident{{\$}SGPtmp1}}{:doSGPtmp1}}\\{{\nwixident{{\$}SGPtmp2}}{:doSGPtmp2}}\\{{\nwixident{{\$}SGPtmpG}}{:doSGPtmpG}}}\nwindexuse{\nwixident{{\$}Loc2}}{:doLoc2}{NWsgp9-graE-1}\nwindexuse{\nwixident{{\$}ofn}}{:doofn}{NWsgp9-graE-1}\nwindexuse{\nwixident{{\$}savefiles{\_}flg}}{:dosavefiles:unflg}{NWsgp9-graE-1}\nwindexuse{\nwixident{{\$}SGPtmp1}}{:doSGPtmp1}{NWsgp9-graE-1}\nwindexuse{\nwixident{{\$}SGPtmp2}}{:doSGPtmp2}{NWsgp9-graE-1}\nwindexuse{\nwixident{{\$}SGPtmpG}}{:doSGPtmpG}{NWsgp9-graE-1}\nwendcode{}\nwbegindocs{69} %def &build_aplot_file &load_gene_struc

\todo{run aplot}{
\item Reformating for {\Tt{}{\$}SGPtmpG.aln} %$
should be avoided if we can redefine the way {\Tt{}gff2aplot} process the frames for aplot format. Take care of this issue if we modify {\Tt{}gff2aplot} to accept {\Tt{}frame:frame} fields.
}

\nwenddocs{}\nwbegincode{70}\sublabel{NWsgp9-run9.2-1}\nwmargintag{{\nwtagstyle{}\subpageref{NWsgp9-run9.2-1}}}\moddef{run aplot~{\nwtagstyle{}\subpageref{NWsgp9-run9.2-1}}}\endmoddef\nwstartdeflinemarkup\nwusesondefline{\\{NWsgp9-graG-1}}\nwenddeflinemarkup
# building aplot file
&build_aplot_file();
# run aplot
...
\nwused{\\{NWsgp9-graG-1}}\nwidentdefs{\\{{\nwixident{{\$}aplot}}{:doaplot}}}\nwidentuses{\\{{\nwixident{{\$}AplotOptions}}{:doAplotOptions}}\\{{\nwixident{{\$}AplotParam}}{:doAplotParam}}\\{{\nwixident{{\$}AplotProgram}}{:doAplotProgram}}\\{{\nwixident{{\&}go{\_}to{\_}die}}{:amgo:unto:undie}}\\{{\nwixident{{\$}Loc1}}{:doLoc1}}\\{{\nwixident{{\$}Loc2}}{:doLoc2}}\\{{\nwixident{{\$}ps{\_}output}}{:dops:unoutput}}\\{{\nwixident{{\$}SGPtmpG}}{:doSGPtmpG}}\\{{\nwixident{{\$}status}}{:dostatus}}\\{{\nwixident{{\$}verbose{\_}flg}}{:doverbose:unflg}}\\{{\nwixident{{\$}verbose{\_}str}}{:doverbose:unstr}}}\nwindexuse{\nwixident{{\$}AplotOptions}}{:doAplotOptions}{NWsgp9-run9.2-1}\nwindexuse{\nwixident{{\$}AplotParam}}{:doAplotParam}{NWsgp9-run9.2-1}\nwindexuse{\nwixident{{\$}AplotProgram}}{:doAplotProgram}{NWsgp9-run9.2-1}\nwindexuse{\nwixident{{\&}go{\_}to{\_}die}}{:amgo:unto:undie}{NWsgp9-run9.2-1}\nwindexuse{\nwixident{{\$}Loc1}}{:doLoc1}{NWsgp9-run9.2-1}\nwindexuse{\nwixident{{\$}Loc2}}{:doLoc2}{NWsgp9-run9.2-1}\nwindexuse{\nwixident{{\$}ps{\_}output}}{:dops:unoutput}{NWsgp9-run9.2-1}\nwindexuse{\nwixident{{\$}SGPtmpG}}{:doSGPtmpG}{NWsgp9-run9.2-1}\nwindexuse{\nwixident{{\$}status}}{:dostatus}{NWsgp9-run9.2-1}\nwindexuse{\nwixident{{\$}verbose{\_}flg}}{:doverbose:unflg}{NWsgp9-run9.2-1}\nwindexuse{\nwixident{{\$}verbose{\_}str}}{:doverbose:unstr}{NWsgp9-run9.2-1}\nwendcode{}\nwbegindocs{71}\nwdocspar
We have the same problem that we stated from {\Tt{}gff2ps} shell script returned exit value to {\Tt{}system\ call} (see explanation on page~\pageref{status256}). 

\subsubsctn{{\Tt{}gff2ps} config file}

\nwenddocs{}\nwbegincode{72}\sublabel{NWsgp9-sgpE-1}\nwmargintag{{\nwtagstyle{}\subpageref{NWsgp9-sgpE-1}}}\moddef{sgp2-gff2ps.rc~{\nwtagstyle{}\subpageref{NWsgp9-sgpE-1}}}\endmoddef\nwstartdeflinemarkup\nwenddeflinemarkup
# 
# sgp2-gff2ps.rc - config file for gff2ps plots on sgp2.
#
# $Id: sgp2.tex,v 1.10 2000-10-19 11:31:29 jabril Exp $ 
#
\LA{}gff2ps set layout~{\nwtagstyle{}\subpageref{NWsgp9-gffH-1}}\RA{}
\LA{}gff2ps set sources~{\nwtagstyle{}\subpageref{NWsgp9-gffI-1}}\RA{}
\LA{}gff2ps set groups~{\nwtagstyle{}\subpageref{NWsgp9-gffH.2-1}}\RA{}
\LA{}gff2ps set features~{\nwtagstyle{}\subpageref{NWsgp9-gffJ-1}}\RA{}
\nwnotused{sgp2-gff2ps.rc}\nwendcode{}\nwbegindocs{73}\nwdocspar
\nwenddocs{}\nwbegincode{74}\sublabel{NWsgp9-gffH-1}\nwmargintag{{\nwtagstyle{}\subpageref{NWsgp9-gffH-1}}}\moddef{gff2ps set layout~{\nwtagstyle{}\subpageref{NWsgp9-gffH-1}}}\endmoddef\nwstartdeflinemarkup\nwusesondefline{\\{NWsgp9-sgpE-1}}\nwenddeflinemarkup
# L # Page Layout
page_size=a4
...
\nwused{\\{NWsgp9-sgpE-1}}\nwendcode{}\nwbegindocs{75}\nwdocspar
\nwenddocs{}\nwbegincode{76}\sublabel{NWsgp9-gffI-1}\nwmargintag{{\nwtagstyle{}\subpageref{NWsgp9-gffI-1}}}\moddef{gff2ps set sources~{\nwtagstyle{}\subpageref{NWsgp9-gffI-1}}}\endmoddef\nwstartdeflinemarkup\nwusesondefline{\\{NWsgp9-sgpE-1}}\nwenddeflinemarkup
# S # Sources
...
\nwused{\\{NWsgp9-sgpE-1}}\nwendcode{}\nwbegindocs{77}\nwdocspar
\nwenddocs{}\nwbegincode{78}\sublabel{NWsgp9-gffH.2-1}\nwmargintag{{\nwtagstyle{}\subpageref{NWsgp9-gffH.2-1}}}\moddef{gff2ps set groups~{\nwtagstyle{}\subpageref{NWsgp9-gffH.2-1}}}\endmoddef\nwstartdeflinemarkup\nwusesondefline{\\{NWsgp9-sgpE-1}}\nwenddeflinemarkup
# G # Groups
*::group_line=arrow
\nwused{\\{NWsgp9-sgpE-1}}\nwendcode{}\nwbegindocs{79}\nwdocspar
\nwenddocs{}\nwbegincode{80}\sublabel{NWsgp9-gffJ-1}\nwmargintag{{\nwtagstyle{}\subpageref{NWsgp9-gffJ-1}}}\moddef{gff2ps set features~{\nwtagstyle{}\subpageref{NWsgp9-gffJ-1}}}\endmoddef\nwstartdeflinemarkup\nwusesondefline{\\{NWsgp9-sgpE-1}}\nwenddeflinemarkup
# F # Features
...
\subsubsctn{{\Tt{}aplot} config file}

\nwenddocs{}\nwbegincode{82}\sublabel{NWsgp9-sgpD-1}\nwmargintag{{\nwtagstyle{}\subpageref{NWsgp9-sgpD-1}}}\moddef{sgp2-aplot.rc~{\nwtagstyle{}\subpageref{NWsgp9-sgpD-1}}}\endmoddef\nwstartdeflinemarkup\nwenddeflinemarkup
# 
# sgp2-aplot.rc - config file for aplot plots on sgp2.
#
# $Id: sgp2.tex,v 1.10 2000-10-19 11:31:29 jabril Exp $ 
#
\LA{}gff2aplot set flags~{\nwtagstyle{}\subpageref{NWsgp9-gffJ.2-1}}\RA{}
\LA{}gff2aplot set defaults~{\nwtagstyle{}\subpageref{NWsgp9-gffM-1}}\RA{}
\LA{}gff2aplot set options~{\nwtagstyle{}\subpageref{NWsgp9-gffL-1}}\RA{}
\LA{}gff2aplot set features~{\nwtagstyle{}\subpageref{NWsgp9-gffM.2-1}}\RA{}
\LA{}gff2aplot set special~{\nwtagstyle{}\subpageref{NWsgp9-gffL.2-1}}\RA{}
\nwnotused{sgp2-aplot.rc}\nwendcode{}\nwbegindocs{83}\nwdocspar
\nwenddocs{}\nwbegincode{84}\sublabel{NWsgp9-gffJ.2-1}\nwmargintag{{\nwtagstyle{}\subpageref{NWsgp9-gffJ.2-1}}}\moddef{gff2aplot set flags~{\nwtagstyle{}\subpageref{NWsgp9-gffJ.2-1}}}\endmoddef\nwstartdeflinemarkup\nwusesondefline{\\{NWsgp9-sgpD-1}}\nwenddeflinemarkup
##
FL ################# FLAGs
##
Display_TITLE             : Y
...
Zoom_Marks                : N
\nwused{\\{NWsgp9-sgpD-1}}\nwendcode{}\nwbegindocs{85}\nwdocspar
\nwenddocs{}\nwbegincode{86}\sublabel{NWsgp9-gffM-1}\nwmargintag{{\nwtagstyle{}\subpageref{NWsgp9-gffM-1}}}\moddef{gff2aplot set defaults~{\nwtagstyle{}\subpageref{NWsgp9-gffM-1}}}\endmoddef\nwstartdeflinemarkup\nwusesondefline{\\{NWsgp9-sgpD-1}}\nwenddeflinemarkup
##
DF ################# DEFAULT VALUES
##
PAGE_SIZE               :  a4
...
\nwused{\\{NWsgp9-sgpD-1}}\nwendcode{}\nwbegindocs{87}\nwdocspar
\nwenddocs{}\nwbegincode{88}\sublabel{NWsgp9-gffL-1}\nwmargintag{{\nwtagstyle{}\subpageref{NWsgp9-gffL-1}}}\moddef{gff2aplot set options~{\nwtagstyle{}\subpageref{NWsgp9-gffL-1}}}\endmoddef\nwstartdeflinemarkup\nwusesondefline{\\{NWsgp9-sgpD-1}}\nwenddeflinemarkup
##
OP ################# OPTIONs
##
Align_NAME            : ##DEFAULT##
...
\nwused{\\{NWsgp9-sgpD-1}}\nwendcode{}\nwbegindocs{89}\nwdocspar
\nwenddocs{}\nwbegincode{90}\sublabel{NWsgp9-gffM.2-1}\nwmargintag{{\nwtagstyle{}\subpageref{NWsgp9-gffM.2-1}}}\moddef{gff2aplot set features~{\nwtagstyle{}\subpageref{NWsgp9-gffM.2-1}}}\endmoddef\nwstartdeflinemarkup\nwusesondefline{\\{NWsgp9-sgpD-1}}\nwenddeflinemarkup
##
FT ################# GFF FEATUREs DEFINITION
##
exon   N   F   R   Y   N   N   N   3   green   lightgreen
\nwused{\\{NWsgp9-sgpD-1}}\nwendcode{}\nwbegindocs{91}\nwdocspar
\nwenddocs{}\nwbegincode{92}\sublabel{NWsgp9-gffL.2-1}\nwmargintag{{\nwtagstyle{}\subpageref{NWsgp9-gffL.2-1}}}\moddef{gff2aplot set special~{\nwtagstyle{}\subpageref{NWsgp9-gffL.2-1}}}\endmoddef\nwstartdeflinemarkup\nwusesondefline{\\{NWsgp9-sgpD-1}}\nwenddeflinemarkup
##
...
This short script was written to test {\sgp} on Sanger Center data-set ({\Tt{}scimog}). Below are shown the steps we followed to do that test.

\nwenddocs{}\nwbegincode{96}\sublabel{NWsgp9-tesB-1}\nwmargintag{{\nwtagstyle{}\subpageref{NWsgp9-tesB-1}}}\moddef{testsgp2.sh~{\nwtagstyle{}\subpageref{NWsgp9-tesB-1}}}\endmoddef\nwstartdeflinemarkup\nwenddeflinemarkup
...
\subsctn{Running {\Tt{}sgp1}}

Here we will draft a shell script to obtain results from {\Tt{}sgp1} program, kindly provided by Thomas Wiehe, and compiled for \textsc{Irix} on apolo.

\nwenddocs{}\nwbegincode{98}\sublabel{NWsgp9-runC.2-1}\nwmargintag{{\nwtagstyle{}\subpageref{NWsgp9-runC.2-1}}}\moddef{runsgp1-a.sh~{\nwtagstyle{}\subpageref{NWsgp9-runC.2-1}}}\endmoddef\nwstartdeflinemarkup\nwenddeflinemarkup
#
# $Id: sgp2.tex,v 1.10 2000-10-19 11:31:29 jabril Exp $
#
...
exit 0;
\nwnotused{runsgp1-a.sh}\nwendcode{}\nwbegindocs{99}\nwdocspar

\nwenddocs{}\nwbegincode{100}\sublabel{NWsgp9-tesB.2-1}\nwmargintag{{\nwtagstyle{}\subpageref{NWsgp9-tesB.2-1}}}\moddef{testsgp1.sh~{\nwtagstyle{}\subpageref{NWsgp9-tesB.2-1}}}\endmoddef\nwstartdeflinemarkup\nwenddeflinemarkup
# 
# Must run on apolo.
#
setenv SGP /home3/jabril/development/sgp/Devel
...
\nwnotused{testsgp1.sh}\nwendcode{}\nwbegindocs{101}\nwdocspar
%$

\subsctn{Comparing results}

The basic {\Tt{}perl} script yields similar results as the previous {\Tt{}shell}, maybe faster. Next to improve is exon boundaries definition, to do that we should include {\Tt{}GetExonAlignment.awk} into our {\sgp} program.\\[2ex]

\begin{tabular}{cc}

\raisebox{-1.25ex}[0pt]{\shortstack{Results Rode\\(old {\Tt{}sgp2})\\{\scriptsize (Aug 2000)}}} &

  {\Tt{}\ 512\ \ 477\ \ 403\ \ 0.96\ \ 0.97\ \ 0.96\ \ 0.96\ \ 0.79\ \ 0.84\ \ 0.82\ \ 0.08\ \ 0.01\ } \\[4ex]

\raisebox{-1.25ex}[0pt]{\shortstack{Results {\Tt{}sgp1}\\{\scriptsize (Oct 2000)}}} &

  {\Tt{}\ 512\ \ 541\ \ 318\ \ 0.81\ \ 0.92\ \ 0.85\ \ 0.85\ \ 0.62\ \ 0.59\ \ 0.60\ \ 0.12\ \ 0.12\ } \\[4ex]

\raisebox{-1.25ex}[0pt]{\shortstack{Results {\Tt{}sgp2}\\{\scriptsize (Oct 2000)}}} &
...
\nwixlogsorted{c}{{gff2aplot set defaults}{NWsgp9-gffM-1}{\nwixu{NWsgp9-sgpD-1}\nwixd{NWsgp9-gffM-1}}}%
\nwixlogsorted{c}{{gff2aplot set features}{NWsgp9-gffM.2-1}{\nwixu{NWsgp9-sgpD-1}\nwixd{NWsgp9-gffM.2-1}}}%
\nwixlogsorted{c}{{gff2aplot set flags}{NWsgp9-gffJ.2-1}{\nwixu{NWsgp9-sgpD-1}\nwixd{NWsgp9-gffJ.2-1}}}%
\nwixlogsorted{c}{{gff2aplot set options}{NWsgp9-gffL-1}{\nwixu{NWsgp9-sgpD-1}\nwixd{NWsgp9-gffL-1}}}%
\nwixlogsorted{c}{{gff2aplot set special}{NWsgp9-gffL.2-1}{\nwixu{NWsgp9-sgpD-1}\nwixd{NWsgp9-gffL.2-1}}}%
\nwixlogsorted{c}{{gff2ps set features}{NWsgp9-gffJ-1}{\nwixu{NWsgp9-sgpE-1}\nwixd{NWsgp9-gffJ-1}}}%
\nwixlogsorted{c}{{gff2ps set groups}{NWsgp9-gffH.2-1}{\nwixu{NWsgp9-sgpE-1}\nwixd{NWsgp9-gffH.2-1}}}%
\nwixlogsorted{c}{{gff2ps set layout}{NWsgp9-gffH-1}{\nwixu{NWsgp9-sgpE-1}\nwixd{NWsgp9-gffH-1}}}%
\nwixlogsorted{c}{{gff2ps set sources}{NWsgp9-gffI-1}{\nwixu{NWsgp9-sgpE-1}\nwixd{NWsgp9-gffI-1}}}%
\nwixlogsorted{c}{{global subroutines}{NWsgp9-gloI-1}{\nwixu{NWsgp9-sgp7-1}\nwixd{NWsgp9-gloI-1}\nwixd{NWsgp9-gloI-2}\nwixd{NWsgp9-gloI-3}\nwixd{NWsgp9-gloI-4}\nwixd{NWsgp9-gloI-5}}}%
\nwixlogsorted{c}{{graphical output}{NWsgp9-graG-1}{\nwixu{NWsgp9-sgp7-1}\nwixd{NWsgp9-graG-1}}}%
\nwixlogsorted{c}{{graphical subs}{NWsgp9-graE-1}{\nwixu{NWsgp9-graG-1}\nwixd{NWsgp9-graE-1}}}%
...
\nwixlogsorted{c}{{runsgp1-a.sh}{NWsgp9-runC.2-1}{\nwixd{NWsgp9-runC.2-1}}}%
\nwixlogsorted{c}{{runsgp2-a.sh}{NWsgp9-runC-1}{\nwixd{NWsgp9-runC-1}}}%
\nwixlogsorted{c}{{set flags-vars}{NWsgp9-setE-1}{\nwixu{NWsgp9-getB-1}\nwixd{NWsgp9-setE-1}\nwixd{NWsgp9-setE-2}}}%
\nwixlogsorted{c}{{sgp2-aplot.rc}{NWsgp9-sgpD-1}{\nwixd{NWsgp9-sgpD-1}}}%
\nwixlogsorted{c}{{sgp2-gff2ps.rc}{NWsgp9-sgpE-1}{\nwixd{NWsgp9-sgpE-1}}}%
\nwixlogsorted{c}{{sgp2.pl}{NWsgp9-sgp7-1}{\nwixd{NWsgp9-sgp7-1}}}%
\nwixlogsorted{c}{{shebang}{NWsgp9-she7-1}{\nwixu{NWsgp9-sgp7-1}\nwixd{NWsgp9-she7-1}}}%
\nwixlogsorted{c}{{testsgp1.sh}{NWsgp9-tesB.2-1}{\nwixd{NWsgp9-tesB.2-1}}}%
