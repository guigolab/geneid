      ...
      if (scoreMarkov >= p->OligoCutoff) 
		{
		  ...
		  /* ---> what follows is changed from default ScoreExons.c */
		  if (SRP)
			{
			  /* SR score */
			  scoreSR = ScoreSRexon((Exons+i),Strand,sr) * TMPBX;
			  /* recompute EW as a function of degree of overlap to HSPs;*/
			  ExonWeight+= x * (1 - (Sumlint/lExon));
			}
		  
		  /* 3. Total (combined) score */
          scoreTotal  =
            1.05*(((Exons+i)->Acceptor->Score
             + (Exons+i)->Donor->Score)/LOG2)
            + 0.95*(scoreMarkov/LOG2)
            + scoreSR;

		  /* ---> changed to here from default ScoreExons.c */  
                ...
                packGC* GCInfo)
{
  ...
  /* Backup of i-counter for each frame */
  for(i=0; i<FRAMES*STRANDS; i++)
	{
	  /*  sprintf(mess,"STARTING: FRAME %ld: SR number %ld/%ld (%ld-%ld-%f)", */
/*  			  i, */
/*  			  sr->iRegions[i], */
/*  			  sr->nRegions[i], */
/*  			  sr->sRegions[i][sr->iRegions[i]].Pos1, */
/*  			  sr->sRegions[i][sr->iRegions[i]].Pos2, */
/*  			  sr->sRegions[i][sr->iRegions[i]].Score); */
/*  	  printMess(mess); */
	  tmpFrames[i] = sr->iRegions[i];
	}
  
  /* First don't need backup of i-counters */
  ...
  /* INTER: Restore of i-counter for each frame */
  for(i=0; i<FRAMES*STRANDS; i++)
	sr->iRegions[i] = tmpFrames[i];
  ...
  /* TERM: Restore of i-counter for each frame */
  for(i=0; i<FRAMES*STRANDS; i++)
	sr->iRegions[i] = tmpFrames[i];
  ...
  /* SINGLES: Restore of i-counter for each frame */
  for(i=0; i<FRAMES*STRANDS; i++)
	sr->iRegions[i] = tmpFrames[i];
  ...
  if (scanORF)
    {
	  /* Restore of i-counter for each frame */
	  for(i=0; i<FRAMES*STRANDS; i++)
		sr->iRegions[i] = tmpFrames[i];
  ...
  /* FORWARD sense */
    if (Strand == FORWARD)
	{
	  /* limit: discarding to this point the SRs */
	  limit = (l2==strlen(Sequence))? l2: l2 - OVERLAP;
	  for(i=0; i<FRAMES; i++)
		{
		  j = tmpFrames[i];
		  while(j < sr->nRegions[i] &&  limit >= sr->sRegions[i][j].Pos2)  
			j++;
		  sr->iRegions[i] = j;
		}
	}
	...
	{
	  /* REVERSE sense */
	  /* limit: jumping up until reaching the first SR out of this region */
	  limit = l1 - (LENGTHSi - OVERLAP);
	  limit = (limit < 0)? 0: limit;
	  for(i=3; i<FRAMES*STRANDS; i++)
		{
		  j = tmpFrames[i];
		  while(j >= 0 &&  limit <= sr->sRegions[i][j].Pos2)  
			j--;
		  sr->iRegions[i] = j;
		}	  
	}
...
/*    for(i=0; i<FRAMES*STRANDS; i++) */
/*  	{ */
/*  	  sprintf(mess,"FINISHING 1: FRAME %ld: SR number %ld/%ld (%ld-%ld-%f)", */
/*  			  i, */
/*  			  sr->iRegions[i], */
/*  			  sr->nRegions[i], */
/*  			  sr->sRegions[i][sr->iRegions[i]].Pos1, */
