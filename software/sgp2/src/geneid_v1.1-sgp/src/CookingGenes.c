...
/*  $Id: CookingGenes.c,v 1.1 2003-09-10 14:53:34 gparra Exp $  */
  ...
  while (!stop)
    {
      ...
      if (!strcmp(e->Type,"Single") || !strcmp(e->Type,"Annotation"))
		{
		  info[igen].start = e;
		  info[igen].end = e;
		  info[igen].nexons = 1;
		  ...
		  if (e->Strand == '-')
			{
			  ...
			  stop1 = (!strcmp(e->Type,"First") || 
					   ...
					   !strcmp(e->Type,"Annotation") || //Genis
					   e->Strand == '+'); 
			  while( !stop && !stop1 )
				{  
				  info[igen].nexons++;
                  ...
                  dict* dAA)
{
  ...
  for(igen=ngen-1; igen>=0; igen--)
    {
      /* find if current gene was passed as evidence */
      annot_flg = strcmp(info[igen].start->Type,"Annotation");
      ...
      if (XML)
		...
		if (!(GFF))
		  {
			/* cDNA */
			if (cDNA && strcmp("Annotation",info[igen].start->Type)) //Genis
			  printProt(Name,ngen-igen,tmpDNA,nNN,cDNA);
			
			/* Protein in FASTA format (except Annotation) */
			if (annot_flg) //Genis
			  printProt(Name,ngen-igen,prot,nAA,PROT);
			else
			  if (!cDNA)
