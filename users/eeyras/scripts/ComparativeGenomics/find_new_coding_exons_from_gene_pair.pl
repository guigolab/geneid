...
foreach my $e2 (@exons2){
    ...
    my $e_id2 = $e2->transcript_tag."\t".$e2->start."-".$e2->end.".".$e2->strand."\t".$pos2->{$e2}."\t".$cod_flag2->{$e2};
	...
	foreach my $pair ( @{$adj_list{$e2}} ){
	    ...
	    my $e_id1 = $pair->transcript_tag."\t".$pair->start."-".$pair->end.".".$pair->strand."\t".$pos1->{$pair}."\t".$cod_flag1->{$pair};
