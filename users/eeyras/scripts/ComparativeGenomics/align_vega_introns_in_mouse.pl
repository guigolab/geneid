...
my $synteny_file  = $ARGV[1];

# species we are comparing against
# mouse
# rat
# chicken
# ...
...
unless ( $encode_region && $synteny_file ){
    print STDERR "Usage: $0 encode.region synteny.file\n";
    exit(0);
}
...
foreach my $intron (@$introns){
	...
	push (@h_exons, $st );
	push (@h_flags, $flag->{$he});
    }
    my $h_exons = join ",",@h_exons;
    my $h_flag  = join ",",@h_flags;
    my @h = ($encode_region,
	     $intron->dbID,
	     "exons:",
	     $h_exons,
