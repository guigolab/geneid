...
######################################################################
#
#  If you use this module in your analyses, please cite:
#
...
It inherits from Root and Range. A TranscriptCluster is a range in the sense that it convers
...
sub new {
  ...
  if ($whatever){
    $self->throw( "Can't pass an object to new()");
  }
...
sub intersection{
  ...
  if ( scalar( @{$self->get_Transcripts} ) == 0 ){
    ...
    my $empty_cluster = ClusterMerge::TranscriptCluster->new();
...
my $union_cluster = ClusterMerge::TranscriptCluster->new();
my $union_strand;
...
sub put_Transcripts {
  ...
  if ( !$new_transcripts[0]->isa('ClusterMerge::Transcript') ){
    $self->throw( "Can't accept a [ $new_transcripts[0] ] instead of a ClusterMerge::Transcript");
  }
...
sub get_unique_Exons{
    my ($self) = @_;
    return GeneComparison::ExonComparison->get_unique_Exons($self->get_Transcripts);
}
...
sub get_Exons{
    ...
    my @exons;
    my %e2t;
    foreach my $t ( @{$self->get_Transcripts} ){
...
sub get_Exons_and_positions{
	...
	my $nt = ClusterMerge::TranscriptUtils->_difuse_small_introns($t,10);
	
	my @exons = sort {$a->start <=> $b->start} @{$nt->get_all_Exons};
	

	#print scalar(@exons)." exons\n";
	my $count = 1;
	foreach my $e (@exons){
	    if ( $e == $exons[0] && $e == $exons[-1] ){
		$pos{$e} = "single";
	    }
	    elsif ( $e == $exons[0] ){
		#print "exon[0] strand = ". $e->strand."\n";
		if ( $e->strand == 1 ){
		    $pos{$e} = "5prime";
		}
		else{
	    ...
	    push( @all_exons, $e );
	    $e2t{$e} = $nt;
	    #print $count."\t".$e2t{$e}->dbID."\tpos = $pos{$e}\n";
	    $count++;
	}
    }
    return (\@all_exons,\%e2t,\%pos);
}

############################################################

sub get_unique_Exons_and_positions{
    my ($self) = @_;
    ...
    my @exons = sort  {$a->start <=> $b->start} GeneComparison::ExonComparison->get_unique_Exons(\@trans);
    my @all_exons;
    my %pos;
    foreach my $e (@exons){
	if ( $e == $exons[0] && $e == $exons[-1] ){
    ...
    return (\@all_exons,\%pos);
}

############################################################

sub get_unique_Exons_and_positions_and_coding_flag{
    my ($self) = @_;
    ...
    my ($exons,$cod_flag) = GeneComparison::ExonComparison->get_unique_Exons_with_coding_flag(\@trans);
...
sub get_coding_Exons{
    ...
    my %e2t;
    foreach my $t ( @{$self->get_Transcripts} ){
	my $ct = $t->coding_transcript;
	foreach my $e (@{$ct->get_all_Exons}){
	    push( @exons, $e );
	    $e2t{$e} = $t;
	}
    }
