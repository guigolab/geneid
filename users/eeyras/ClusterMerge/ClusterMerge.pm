...
=head1 SYNOPSIS
    ...
    my $cluster_merge = ClusterMerge::CusterMerge->new(
...
package ClusterMerge::ClusterMerge;
...
sub link_Transcripts{
 ...
 CLUSTER:
  ...
  TRANSCRIPT:
	...
	# put this in a node
	my $newnode = ClusterMerge::Node->new();
    ...
    #      foreach my $leaf ( @$all_the_leaves ){
    #	print STDERR "leaf:\n";
    #	ClusterMerge::TranscriptUtils->_print_SimpleTranscript($leaf->transcript);
    #	foreach my $parent ( @{$leaf->extension_parents} ){
    #	  print STDERR "extension parent:\n";
    #	  ClusterMerge::TranscriptUtils->_print_SimpleTranscript($parent->transcript);
    #	}
    #      }
    #    }
  ...
  ############################################################
  # convert the ClusterMerge::Node objects
  # back into ClusterMerge::Transcript objects
  ############################################################
  my @lists;
...
sub compare{
  ...
  if ( $self->verbose ){
      ...
      #print STDERR "         intron mismatch: ".$self->_intron_mismatch."\n";
      #print STDERR "         splice mismatch: ".$self->_splice_mismatch."\n";
      #print STDERR "internal splice mismatch: ".$self->_internal_splice_overlap."\n";
  }
...
sub run {
  ...
  ############################################################
  # if comp level is 5 we difuse the transcripts
  if ( $self->_comparison_level == 5 && $self->_intron_mismatch ){
      foreach my $tran ( $self->input_transcripts ){
	  push( @transcripts, ClusterMerge::TranscriptUtils->_difuse_small_introns( $tran, $self->_intron_mismatch ) );
      }
  }
  else{
      @transcripts = $self->input_transcripts;
  }
...
sub _filter_clusters{
 ...
 CLUSTER:
  foreach my $cluster ( @$clusters ){
    my $new_cluster = ClusterMerge::TranscriptCluster->new();
    my @transcripts = 
...
=head2 _cluster_Transcripts
 ...
 Description : It separates transcripts according to strand and then clusters 
  ...
  Args       : Array of ClusterMerge::Transcript
  Return     : Array of ClusterMerge::TranscriptCluster
...
sub _merge_Transcripts{
  ...
  LIST:
	...
	for (my $i = 0; $i< scalar( @exons ); $i++){
      ...
      my $exon_count = 0;
      ############################################################
      # create an exon per exon cluster
      foreach my $exon_cluster (@exon_clusters){
	  $exon_count++;
	  
	  my @exons_in_cluster = @{$exon_cluster->get_Exons};
	  
	  my $new_exon = ClusterMerge::Exon->new();	  
	  $new_exon->start($exon_cluster->start );
	  $new_exon->end  ($exon_cluster->end   );
	  $new_exon->dbID( $transcript->type."_".$exon_count);
	  $new_exon->seqname( $exons_in_cluster[0]->seqname );
	  $new_exon->score(100);
	  
	  ############################################################
	  # store evidence
	  $new_exon->exon_evidence(@{$exon_cluster->get_Exons});
	  ...
	  my %evidence_hash;
	  my %evidence_obj;
	  #foreach my $exon ( @{$exon_cluster->get_Exons} ){
...
sub _set_splice_Ends {
 ...
 foreach my $cluster ( @exon_clusters ){
   ...
   push( @exon_list, @{$cluster->get_Exons});
   ...
   #     my @exons = sort{ $b->length <=> $a->length } @{$cluster->get_Exons};
...
sub _transfer_Supporting_Evidence{
 ...
 SOURCE_FEAT:
  foreach my $feat (@{$source_exon->get_all_supporting_features}){
    next SOURCE_FEAT unless $feat->isa("ClusterMerge::FeaturePair");
   ...
   TARGET_FEAT:
     foreach my $tsf (@$target_sf){
       #next TARGET_FEAT unless $tsf->isa("ClusterMerge::FeaturePair");
