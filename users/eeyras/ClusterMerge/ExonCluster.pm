...
=head2 intersection
  ...
  Title   : intersection
  Usage   : ($start, $end) = $exon_cluster1->intersection($exon_cluster2)
...
sub intersection{
    ...
    my ($start1,$end1) = ( $c1->start, $c1->end );
    my ($start2,$end2) = ( $c2->start, $c2->end );   
    
    my $end   = $c1->min( $end1,  $end2 );
    my $start = $c2->max( $start1, $start2);
