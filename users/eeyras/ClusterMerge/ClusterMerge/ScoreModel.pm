...
package ClusterMerge::ScoreModel;
...
sub get_alternative_sites{
  ...
  foreach my $tran ( @trans ){
    foreach my $exon ( @{$tran->get_all_Exons} ){
      ...
      #print STDERR "exon2transcript{$exon} = $exon2transcript{$exon} = $tran\n";
      push( @all_exons, $exon );
    }
  }
  ...
  my $exon_cluster_list = ClusterMerge::ExonUtils->_cluster_Exons( @all_exons );
  my @clusters = sort { $a->start <=> $b->start } @{$exon_cluster_list};
 ...
 EXON_CLUSTER:
  foreach my $exon_cluster ( @clusters ){
    $exon_cluster_pos++;
    my @exons =@{$exon_cluster->get_Exons};
    my $previous_exon;
    my %seen_transcript;
  ...
  EXON:
    ...
    TRAN:
      foreach my $t ( @trans ){
	#print STDERR "exon2trans{$exon}: ".$exon2transcript{ $exon }." looking at trans: $t\n";
