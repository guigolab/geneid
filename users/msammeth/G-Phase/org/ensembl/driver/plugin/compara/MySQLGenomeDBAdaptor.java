...
**/
public class MySQLGenomeDBAdaptor extends ComparaBaseAdaptor implements GenomeDBAdaptor{
  ...
  public MySQLGenomeDBAdaptor(ComparaMySQLDriver driver) {
    super(driver);
  ...
  **/
  ...
  /**
   * Return the genomedb with the input species name, if there's only one
   * there. Otherwise throw an adaptor exception.
  **/
  public GenomeDB fetch(String speciesName) throws AdaptorException {
    ...
    Iterator keys = getGenomeDBCacheByNameAssembly().keySet().iterator();
    String key = null;
    String relevantKey = null;
    while(keys.hasNext()){
      key = (String)keys.next();
      if(key.indexOf(speciesName) >= 0){
        countOfInstance++;
        relevantKey = key;
      }//end if
    }//end while
    ...
    if(countOfInstance == 1){
      return (GenomeDB)getGenomeDBCacheByNameAssembly().get(relevantKey);
    }else{
      throw new AdaptorException(
  ...
  private void fillCache() throws AdaptorException{
    ...
    try {
      ...
      throw new AdaptorException("Problems reading from genome_db", sqle);
    }finally{
      close(connection);
    }//end try
  ...
  **/
  protected void mapColumnsToObject(HashMap columns, Persistent object) {
    ...
    genomeDB.setInternalID(((Integer)columns.get(GENOME_DB_ID)).intValue());
    genomeDB.setTaxonId(((Integer)columns.get(TAXON_ID)).intValue());
