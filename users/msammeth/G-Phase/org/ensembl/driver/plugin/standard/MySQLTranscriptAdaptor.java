 ...
 *
 * Most of the work is done in MySQLBaseFeatureAdaptor.
 */

public class MySQLTranscriptAdaptor
  ...
  private static final Logger logger =
    Logger.getLogger(MySQLTranscriptAdaptor.class.getName());

  private MySQLGeneAdaptor geneAdaptor;
  private MySQLExonAdaptor exonAdaptor;
  private MySQLTranslationAdaptor translationAdaptor;

  private int[] numExonsPerTranscript;
  public MySQLTranscriptAdaptor(MySQLDriver driver) {

    super(driver, TYPE);

    try {

      geneAdaptor = (MySQLGeneAdaptor) driver.getGeneAdaptor();
      exonAdaptor = (MySQLExonAdaptor) driver.getExonAdaptor();
      translationAdaptor =
        (MySQLTranslationAdaptor) driver.getTranslationAdaptor();

    } catch (AdaptorException ae) {
      System.err.println("Error creating gene adaptor");
      ae.printStackTrace();
    }
  }

  //	-----------------------------------------------------------------
  // Implementation of required methods from MySQLBaseFeatureAdaptor
  ...
  /*
   * @see org.ensembl.driver.plugin.standard.MySQLBaseFeatureAdaptor#leftJoin()
   */
  ...
  private void loadTranslations(List transcripts) throws AdaptorException {
    ...
    List translations =
      translationAdaptor.fetchByTranscripts(transcriptIDs, false);
  ...
  public Transcript fetchByTranslation(long translationID)
    ...
    try {
      ...
      ResultSet rs = executeQuery(conn, sql);
      if (rs.next()) {
        long id = rs.getLong("transcript_id");
        t = fetch(id);
        ;
      }
    } catch (Exception se) {
  ...
  /** 
   * @return transcript internalID 
   */
  private long storeTranscript(
    Connection conn,
    StringBuffer sql,
