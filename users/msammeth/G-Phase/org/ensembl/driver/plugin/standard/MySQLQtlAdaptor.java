 ...
 *
 */
public class MySQLQtlAdaptor
  extends MySQLBaseFeatureAdaptor
  implements QtlAdaptor {

  public MySQLQtlAdaptor(MySQLDriver driver) {
    super(driver, TYPE);
  }

  public Qtl fetch(long internalID) throws AdaptorException {
    return (Qtl) fetchByNonLocationConstraint("q.qtl_id = " + internalID).get(
      0);
  }
  ...
  public Object createObject(ResultSet rs) throws AdaptorException {
    ...
    try {
      ...
      while (rs.next()) {
        ...
        long id = rs.getLong("q.qtl_id");
        if (qtl == null) {

          qtl = new QtlImpl(getDriver());
          qtl.setInternalID(id);
          qtl.setTrait(rs.getString("q.trait"));
          qtl.setLodScore(rs.getFloat("q.lod_score"));

          qtl.setFlankMarker1(ma.fetch(rs.getLong("q.flank_marker_id_1")));
          qtl.setFlankMarker2(ma.fetch(rs.getLong("q.flank_marker_id_2")));
          qtl.setPeakMarker(ma.fetch(rs.getLong("q.peak_marker_id")));
        ...
        qtl.getSynoyms().add(
          new QtlSynonymImpl(
            rs.getString("qs.source_database"),
            rs.getString("qs.source_primary_id")));
