...
public class MySQLMarkerAdaptor extends BaseAdaptor implements MarkerAdaptor {
  private static final Logger logger =
    Logger.getLogger(MySQLMarkerAdaptor.class.getName());

  public MySQLMarkerAdaptor(MySQLDriver driver) {
    super(driver);
  }
  ...
  public Marker fetch(long internalID) throws AdaptorException {
    ...
    try {
      if (rs.next()) {
        ...
        m.setInternalID(rs.getLong("m.marker_id"));
        m.setMaxPrimerDistance(rs.getInt("m.max_primer_dist"));
        m.setType(rs.getString("m.type"));
        m.setMinPrimerDistance(rs.getInt("m.min_primer_dist"));
        m.setMaxPrimerDistance(rs.getInt("m.max_primer_dist"));
        m.setSeqLeft("m.left_primer");
        m.setSeqRight("m.right_primer");
        List synonyms = new ArrayList();
        m.setSynonyms(synonyms);
        long displaySynonymID = rs.getLong("m.display_marker_synonym_id");
        do {
          String synonym = rs.getString("ms.name");
          synonyms.add(synonym);
          if ( displaySynonymID==rs.getLong("ms.marker_synonym_id"))
