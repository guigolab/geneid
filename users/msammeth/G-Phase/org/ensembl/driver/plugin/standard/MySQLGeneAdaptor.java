...
public class MySQLGeneAdaptor
  extends MySQLBaseFeatureAdaptor
  implements GeneAdaptor {

  private static final Logger logger =
    Logger.getLogger(MySQLGeneAdaptor.class.getName());

  private MySQLTranscriptAdaptor transcriptAdaptor = null;

  private MySQLExonAdaptor exonAdaptor = null;

  public MySQLGeneAdaptor(MySQLDriver driver) {
  ...
  public Object createObject(ResultSet rs) throws AdaptorException {
    ...
    try {
      ...
      if (rs.next()) {
        ...
        if (loc == null) {
          logger.warning(
            "Warning location is null for gene " + g.getInternalID());
        }

        g = new GeneImpl(rs.getLong("seq_region_id"), loc);
        g.setInternalID(rs.getLong("gene_id"));
        g.setDriver(driver);
        // Possible optmisation: lazy load analysis to speed things up?
        g.setAnalysis(analysisAdaptor.fetch(rs.getLong("analysis_id")));
  ...
  private void loadChildren(List genes) throws AdaptorException {
