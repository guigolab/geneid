...
public class MySQLSequenceAdaptor
  extends BaseAdaptor
  implements SequenceAdaptor {
  private static final Logger logger =
    Logger.getLogger(MySQLSequenceAdaptor.class.getName());

  MySQLLocationConverter converter = null;

  public MySQLSequenceAdaptor(MySQLDriver driver) {
    super(driver);
    try {
      converter = (MySQLLocationConverter) driver.getLocationConverter();
    } catch (AdaptorException ae) {
      System.out.println(
        "Error initialising location convertor in MySQLSequenceAdaptoe");
      ae.printStackTrace();
    }
  }
  ...
  */
    ...
    try {
      ...
      while (cloc != null) {
        ...
        if (cloc.isGap()) {
          ...
          long regionId =
            converter.nameToId(
              cloc.getSeqRegionName(),
              cloc.getCoordinateSystem());
  ...
  public Sequence fetch(Location location) throws AdaptorException {
    ...
    CoordinateSystem seqCS;
    Location seqLevelLocation;
    seqCS = driver.getCoordinateSystemAdaptor().fetchSequenceLevel();
    ...
    seqLevelLocation = driver.getLocationConverter().convert(completeLoc, seqCS);
    String seqString;
    if (compressed) {
