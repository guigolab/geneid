 ...
 * 
 * <p>
 * Works in conjunction with the it's peer MySQLRepeatConsensusAdaptor.
 * </p>
 * 
 * @author <a href="mailto:craig@ebi.ac.uk">Craig Melsopp</a>
 *
 */
public class MySQLRepeatFeatureAdaptor
  extends MySQLBaseFeatureAdaptor
  implements RepeatFeatureAdaptor {

  private static final Logger logger =
    Logger.getLogger(MySQLRepeatFeatureAdaptor.class.getName());
  ...
  */
    ...
    try {
      if (rs.next()) {
        ...
        Location hitLoc =
          new Location(
            repeatCoordSys,
            rs.getString("rc.repeat_name"),
            start,
            end,
            0);

        f =
          new RepeatFeatureImpl(rs.getLong("r.repeat_feature_id"), loc, hitLoc);
        f.setDisplayName("not set");
        f.setDescription("not set");
        f.setSequence(null);
        f.setScore(rs.getDouble("r.score"));
        f.setHitDisplayName(rs.getString("rc.repeat_name"));
        f.setAnalysis(analysisAdaptor.fetch(rs.getLong("r.analysis_id")));

        long rcID = rs.getLong("r.repeat_consensus_id");
        f.setRepeatConsensusInternalID(rcID);
        
        MySQLRepeatConsensusAdaptor rca = (MySQLRepeatConsensusAdaptor) driver.getRepeatConsensusAdaptor();
        ...
        if ( rc==null ) 
          rc = rca.add(rcID, 
                        rs.getString("rc.repeat_name"), 
                        rs.getString("rc.repeat_class"), 
                        rs.getString("rc.repeat_consensus")); 
  ...
  public MySQLRepeatFeatureAdaptor(MySQLDriver driver) {
    super(driver, TYPE);
  }

  public MySQLRepeatFeatureAdaptor(
    MySQLDriver driver,
    String logicName,
    String type) {
