...
import org.ensembl.datamodel.variation.impl.VariationGroupImpl;
 ...
 *
 */
public class MySQLVariationGroupAdaptor extends BasePersistentAdaptor implements VariationGroupAdaptor {

  private final static String BASE_QUERY =
    "SELECT vg.variation_group_id, vg.name, s.name, vg.type, vgv.variation_id"
      + " FROM variation_group vg, source s";

  /**
   * 
   */
  public MySQLVariationGroupAdaptor(VariationDriver vdriver) {
    super(vdriver);
  }

  /**
   * @see org.ensembl.driver.variation.VariationGroupAdaptor#fetch(long)
   */
  ...
  /**
   * @see org.ensembl.driver.variation.VariationGroupAdaptor#fetch(org.ensembl.datamodel.variation.Variation)
   */
  ...
  /**
   * @see org.ensembl.driver.variation.VariationGroup#TYPE
  ...
  protected Object createObject(ResultSet rs) throws SQLException, AdaptorException {
    ...
    do {
      ...
      if (vg == null) {
        ...
        vg.setSource(rs.getString("s.name"));
        vg.setType(rs.getString("vg.type"));
      }
      
      long variationID = rs.getLong("vgv.variation_id");
      if (variationID>0)
