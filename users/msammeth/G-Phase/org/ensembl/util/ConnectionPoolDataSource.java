...
public class ConnectionPoolDataSource implements DataSource {
	...
	private class ConnectionCloser extends Thread{
		...
		/**
 		 * Runs every CONNECTION_POOL_CLOSE_TIMEOUT ms and 
		 * closes all pooled connections if connectionEvent()
		 * has not been called since the last time it ran.
		 */
		public void run() {
			while(true) {
				try {
	...
	private static final Logger logger =
		Logger.getLogger(ConnectionPoolDataSource.class.getName());
	 ...
	 * @param fullConnectionString connection string
	 * @param user user name
	 * @param password password, can be null
	 * @param maxPoolSize maximum connection pool size, must be >0
	 * @throws ClassNotFoundException if the jdbc driver can be found
	 * @throws SQLException if a problem occurs connecting to the database
	...
	public ConnectionPoolDataSource(
		...
		if (maxPoolSize < 1)
			throw new IllegalArgumentException(
				"maxPoolSize should be >0 but is: " + maxPoolSize);
	...
	/**
	 * Gets a connection, either from the pool or a new one is created. If creating a new connection
	 * would exceed the pool size then an SQLException is thrown.
	 * 
	 * @return connection from pool if one is available, otherwise a new one if the creation of the connection does
	 * not exceed the maxPoolSize.
	 * @see javax.sql.DataSource#getConnection()
	 * @throws SQLException if a problem occurs connecting to db. This includes if the number of connections
	 * will be exceeded if another is created.
	 */
		...
		if (conn == null)
			if (active.size() >= maxPoolSize)
				throw new SQLException(
					"Connection pool limit of " + maxPoolSize + "exceeded");
			else
	...
	/**
	 * Returns the connection to the pool. Does nothing if this connection was not created by this instance.
	 * @param conn connection to be returned to pool.
	 */
	...
	private Connection createNewConnection() throws SQLException {
		...
		Connection conn =
			new ConnectionWrapper(
				driver.connect(fullConnectionString, config),
				this);
	...
	/**
	 * 
	 * @return current number of connections that have been given out through getConnection().
	 */
	...
	private synchronized void closeAllConnections(List connections) throws SQLException {
	 ...
	 * handles the fact that the datasource is null and prints a warning
	 * if it is not a ConnectionPoolDataSource.
	 * @param datasource
	 * @throws SQLException
	 */
	public static void closeAllConnections(DataSource datasource) throws AdaptorException {
		if (datasource==null) {
					...
					logger.warning("Can not close connections because unsupported Datasource: " + datasource);
