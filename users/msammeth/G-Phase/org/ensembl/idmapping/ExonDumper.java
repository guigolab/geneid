...
public class ExonDumper {
    ...
    public void dumpFilteredExons(Cache cache, ScoredMappingMatrix matrix, String rootDir) {
        ...
        List sourceExons = filterExons(cache.getSourceExonsByInternalID(), matrix, true);
        writeExons(sourceExons, rootDir + File.separator + System.getProperty("idmapping.source.database"));

        List targetExons = filterExons(cache.getTargetExonsByInternalID(), matrix, false);
     ...
     */
    private List filterExons(Map allExons, ScoredMappingMatrix matrix, boolean isSource) {

        List exons = new ArrayList();

        int totalExons = 0;

        Iterator it = allExons.values().iterator();
        while (it.hasNext()) {
            Exon e = (Exon) it.next();
            totalExons++;
