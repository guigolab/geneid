...
public class TranslationImpl extends PersistentImpl implements Translation {
  ...
  private static final long serialVersionUID = 1L;
			...
			for (int e = 0; e < nExons; ++e) {
				...
				if (codingExon) {
					...
					if (lastExon) {
						...
						final int crop = positionInEndExon
								- exonLoc.getLength();
						if (crop != 0) {
	...
	public Location getAminoAcidStart(int aminoAcidPosition) {
		...
		// Assumes the codingLocations are sorted. e.g. for assembly locations,
		// on positive strand locations[n].start < locations[n+1].start, while
		// on -ve strand
		// locations[n].start > locations[n+1].start

		Location targetLocation = null;

		final int transcriptPosition = (aminoAcidPosition - 1) * 3;

		int remainingBases = transcriptPosition;
		int baseCounter = 0;

		// Walk through exons to find _transcriptPosition_
		final List locations = getCodingLocations();
		final int nLocations = locations.size();

		for (int i = 0; i < nLocations && targetLocation == null; ++i) {

			Location tmp = (Location) locations.get(i);

			final int tmpLen = tmp.getNodeLength();
			baseCounter += tmpLen;

			if (baseCounter >= transcriptPosition) {

				// The location we are looking for is only one base long so it
				// must
				// be limited to a single location object (not being spread over
				// list
				// of them). We clone the one that contains the correct location
				// and
				// shrink it to 1 base pair long.
				final int diff = transcriptPosition - (baseCounter - tmpLen);
				final int endDiff = diff - tmpLen + 1;
				targetLocation = tmp.transformNode(diff, endDiff);
	...
	public synchronized String getPeptide() {
		...
		if (seq != null) {
			...
			peptide = SequenceUtil.dna2protein(dnaSeq);

			// Crop 'stop' amino acid if included
			if (peptide.length() > 0) {

				final int lastPos = peptide.length() - 1;
				if (peptide.charAt(lastPos) == '*') {
					peptide = peptide.substring(0, lastPos);
				}
	...
	private synchronized void lazyLoadKnown() {
		...
		if (driver != null) {
			try {
				...
				logger.warning("Failed to lazy load translation.known. "
						+ e.getMessage());
			}
		}
	}
