 ...
 */
public class TCoffeeWrapper implements AlignmentWrapper {
	...
	public static final String EXEC_PATH= System.getProperty("user.dir")+ File.separator+ "extern";
	...
	public int start() {
		...
		cmd+= EXEC_PATH+ File.separator+ EXEC_NAME;
		cmd+= " "+ fileAbsName;
		cmd+= " -output=gcg";
		cmd+= " -outorder=input";
...
//		command= "command /c "+ command+ " > .\\extern\\stdout.txt";
		...
		try {
			...
			DevPipeReaderThread pipe= new DevPipeReaderThread(proc.getErrorStream(), System.err);
			DevPipeReaderThread pipe2= new DevPipeReaderThread(proc.getInputStream(), System.out);
			pipe.start();
			pipe2.start();
