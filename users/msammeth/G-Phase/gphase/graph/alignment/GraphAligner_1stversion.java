...
public class GraphAligner_1stversion {
	...
	public static void main(String[] args) {
		...
		Mapping_1stversion[] maps= align(g1, g2);
	...
	public GraphAligner_1stversion(SpliceGraph newG1, SpliceGraph newG2) {
		
	}
	
	public static Mapping_1stversion[] align_dp(SpliceGraph g1, SpliceGraph g2) {
		...
		for (int i = 0; i < hh.length; i++) {
			...
			dd[i][0]= Mapping_1stversion.getCost();
			
		}
		
		if (listI.length> 0) {
			Mapping_1stversion m= new Mapping_1stversion();
		...
		Mapping_1stversion map= (Mapping_1stversion) q.poll();
		...
		while (map!= null&& map.getCost()<= ulCost) {
			...
			if (nextI+ 1< listI.length) {
				Mapping_1stversion m= null;
				try {
					m = (Mapping_1stversion) map.clone();
			...
			map= (Mapping_1stversion) q.poll();	// next
		}
		
		return (Mapping_1stversion[]) gphase.tools.Arrays.toField(optMaps);
	}

	public static Mapping_1stversion[] align(SpliceGraph g1, SpliceGraph g2) {
		
		PriorityQueue q= new PriorityQueue(11, new Mapping_1stversion.PriorityComparator());
