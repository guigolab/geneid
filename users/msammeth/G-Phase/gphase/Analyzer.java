...
//			}
	...
	public static void _05_intronModuloDistribution() {
		//fruitfly_BDGP43_EnsEMBL43_dlMart_norman_.gff
		File inFile= new File("/projects/G-phase/ASTALAVISTA/input/human_NCBI36_EnsEMBL43_dlMart_norman_.gff");
		...
		while (g!= null) {
			...
			for (int i = 0; i < g.length; i++) {
				...
				for (int j = 0; j < t.length; j++) {
					if (!t[j].isCoding())
						continue;
					DirectedRegion[] reg= t[j].getIntrons();
					for (int k = 0; reg!= null&& k < reg.length; k++) 
						if (t[j].getTranslations()[0].contains(reg[k])) {
							++nbAll;
							if (reg[k].getLength()% 3== 0)
								++nb3;
						}
				}
			}
		...
		System.out.println(nb3+" out of "+nbAll+" frame perserving ("+Formatter.fprint((100d* nb3)/ nbAll, 2)+")");
	}
	 ...
	 *
	 */
	static void _06_outputAlternativeSS() {
		...
		// /home/msammeth/annotations/hg17_RefSeqGenes_fromUCSC_070611_CDS_mRNAs_fromUCSC_070611_CDS.gtf
		long t0= System.currentTimeMillis();
		gphase.tools.File file= new gphase.tools.File("/home/msammeth/annotations/hg17_RefSeqGenes_fromUCSC_070611_CDS_mRNAs_fromUCSC_070611_CDS.gtf");
		Species species= new Species("human");
		species.setGenomeVersion("hg18");
		
		GTFChrReader reader= new GTFChrReader(file.getAbsolutePath());
		String outFileName= file.getPathOnly()+ File.separator+ file.getFileNameWithoutExtension()+ "_ss."+ file.getExtension();
		...
		while (g!= null) {
			for (int i = 0; i < g.length; i++) {
				if (g[i].getTranscripts().length> 1000) {
					System.out.println("Skipped cluster with "+g[i].getTranscripts().length+" transcripts: "+g[i].getTranscripts()[0].getTranscriptID());
					continue;
				}
				g[i].setSpecies(species);
				ASVariation[] as= g[i].getASVariations(ASMultiVariation.FILTER_STRUCTURALLY, null);	// init
				for (int j = 0; as!= null&& j < as.length; j++) {
					if (!as[j].isASevent())
						as[j].removeFromSpliceSites();
				}
				SpliceSite[] ss= g[i].getSpliceSites();
				if (ss== null|| ss.length== 0)
					continue;
				GTFObject[] obs= new GTFObject[ss.length];  
				for (int j = 0; j < ss.length; j++) {
					if (!ss[j].isSpliceSite())
						continue;
					obs[j]= GTFObject.createGFFObject(ss[j]);
					if (!ss[j].isConstitutive()) {
						StringBuffer sb= new StringBuffer();
						for (int k = 0; k < ss[j].getAsVars().length; k++) { 
							sb.append(ss[j].getAsVars()[k].toStringStructureCodeWithoutSpaces());						
							sb.append('|');
						}
						sb.deleteCharAt(sb.length()- 1);						
						obs[j].addAttribute("events", sb.toString());
					}
				}
				buffy.setGtfObj(obs);
				try {
