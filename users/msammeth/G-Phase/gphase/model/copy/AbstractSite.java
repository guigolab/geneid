...
//	----------------------------------------------
	...
	public Gene getGene() {
		if (transcripts== null|| transcripts.length< 1) {
			if (transHash== null)			
				return null;
			return (transHash.values().iterator().next().getGene());
		}
		return transcripts[0].getGene();
	}
	public Transcript[] getTranscripts() {
		if (transcripts== null&& transHash!= null) {
	...
	public void addTranscripts(HashMap<Transcript, Transcript> newTransHash) {
		if (getGene()== null|| getGene().isConstruct()) {
			transHash= (HashMap<Transcript, Transcript>) newTransHash.clone();
			return;
		} 
		// else 
		Object[] newTrans= newTransHash.keySet().toArray();
		Vector v= new Vector(newTrans.length);
		for (int i = 0; i < newTrans.length; i++) {
			transHash.put((Transcript) newTrans[i], (Transcript) newTrans[i]);
		}
	}
	...
	public void addTranscripts(Transcript[] newTrans) {
		...
		if (getGene()!= null&& getGene().isConstruct()) {
			if (transHash== null)
				transHash= new HashMap<Transcript, Transcript>();
			for (int i = 0; i < newTrans.length; i++) 
				transHash.put(newTrans[i], newTrans[i]);
		} else {
			...
			if (getTranscripts()== null) {
				...
				for (int i = 0; i < newTrans.length; i++) {
					...
					for (j = 0; j < transcripts.length; j++) 
						if (transcripts[j]== newTrans[i])		// equals not less efficient on mRNA?!
							break;
	...
	public boolean addTranscript(Transcript newTrans) {
		...
		if (getGene().isConstruct()) {
			...
			if (transHash.get(newTrans)!= null)
				return false;
			transHash.put(newTrans,newTrans);
			return true;
		} else {
			for (int i = 0; getTranscripts()!= null&& i < transcripts.length; i++) 
