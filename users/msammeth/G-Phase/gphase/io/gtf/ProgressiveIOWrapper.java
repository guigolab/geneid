...
public class ProgressiveIOWrapper extends GTFWrapper {
	...
	public static void main(String[] args) {
		...
		ProgressiveIOWrapper myWrapper= new ProgressiveIOWrapper(new File(fName).getAbsolutePath()); // testGTF.gtf
		try {
			myWrapper.read();
		} catch (Exception e) {
			e.printStackTrace(); 
...
//		}
	...
	public ProgressiveIOWrapper(String absFName) {
		super(absFName);		
	}
	
	public ProgressiveIOWrapper(String absFName, String speName) {
		this(absFName);
		this.speName= speName;
	}
	
	public ProgressiveIOWrapper(String absFName, String speName, String buildVersion) {
		this(absFName, speName);
		this.buildVersion= buildVersion;
	}
	
	public ProgressiveIOWrapper(InputStream i) {
		this.inputStream= i;
	}
...
//		}
	...
	public Species assemble() {
		return ProgressiveIOWrapper.assemble(getGtfObj(), getSpeciesName(), getBuildVersion());
	}
...
//		checkMegaClusters(((Transcript[]) Arrays.toField(v)));
	...
	/**
		 ...
		 * @return
		 */
		public static Gene[] assemble(GTFObject[] gtfObs) {
			...
			for (int i = 0; i < gtfObs.length; i++) {		// exons
				String tid= gtfObs[i].getAttribute(GTFObject.TRANSCRIPT_ID_TAG);
				int strand= gtfObs[i].getStrand();
				if (lastChromID== null)
					lastChromID= gtfObs[i].getChromosome();
				if (!tid.equals(lastTID)|| strand!= lastStrand|| 
						!gtfObs[i].getChromosome().equals(lastChromID)) {
						// create new trpt
					trpt= new Transcript(tid);
					trpt.setStrand(gtfObs[i].getStrand());
					trpt.setChromosome(gtfObs[i].getChromosome());
					trpt.setSource(gtfObs[i].getSource());
					trptV.add(trpt);
					lastTID= tid;
					lastStrand= strand;
				}
				if (gtfObs[i].isExon()) {
					Exon e= new Exon(trpt, gtfObs[i].getExonID(), gtfObs[i].getStart(), gtfObs[i].getEnd());
					if (trpt.getExons()== null|| trpt.getExons().length== 0) {
						e.setChromosome(gtfObs[i].getChromosome());
						if (!e.overlaps(lastGene)) {
							if (lastGene!= null)
								geneV.add(lastGene);
							lastGene= new Gene(Gene.getUniqueID());
							lastGene.setStrand(trpt.getStrand());
							lastGene.setChromosome(gtfObs[i].getChromosome());
						}
						lastGene.addTranscript(trpt);
						trpt.setGene(lastGene);
					}
					trpt.addExon(e);				
				} else if (gtfObs[i].isCDS()) 
					trpt.addCDS(gtfObs[i].getStart(), gtfObs[i].getEnd());
			}
			if (trpt!= null)
				trptV.add(trpt);
			...
			// cluster for chr
	//		HashMap chrHash= new HashMap();
	//		for (int i = 0; i < trptV.size(); i++) {
	//			Vector v= (Vector) chrHash.get(((Transcript) trptV.elementAt(i)).getChromosome());
	//			if (v== null) {
	//				v= new Vector();
	//				chrHash.put(((Transcript) trptV.elementAt(i)).getChromosome(), v);
	//			}
	//			v.add(trptV.elementAt(i));
	//		}
	//		
	//			// cluster for genes
	...
	//			Transcript[] t= (Transcript[]) Arrays.toField(chrHash.get(o[i]));
	//			Transcript[][] loci= clusterTranscripts(t);
	//			for (int j = 0; j < loci.length; j++) {
	//				String gID= Gene.getUniqueID();
	//				Gene locus= new Gene(gID);
	//				locus.setStrand(loci[j][0].getStrand());
	//				locus.setChromosome(loci[j][0].getChromosome());
	//				for (int k = 0; k < loci[j].length; k++) { // transcripts
	//					loci[j][k].setGene(locus);
	//					locus.addTranscript(loci[j][k]);
	//				}
	...
	//		}
			...
			return (Gene[]) Arrays.toField(geneV);
		}
	...
	public Gene getNextGene() {
		...
		GTFObject[] gtfObs= readNextBunch();
			// build transcripts according to order from the input
		// build transcripts according to order from the input
		String lastTID= "";
		int lastStrand= 0;
		...
		try {
			...
			while (buffy.ready()) {
				...
				if (tokens[2].equalsIgnoreCase("exon")) {
					...
					if (trpt.getExons()== null|| trpt.getExons().length== 0) {
						e.setChromosome(tokens[0]);
						if (!e.overlaps(gene)) {
							if (gene!= null)
								break;
							gene= new Gene(Gene.getUniqueID());
							gene.setStrand(trpt.getStrand());
	...
	public boolean isApplicable() {
		...
		try {
			...
			while (buffy.ready()) {
				...
				if (tid== null) {
					System.err.println("No TID found (line"+lineCtr+").");
					continue;
				}
				...
				if (!tid.equals(lastTID)) {
					int start= Integer.parseInt(tokens[3]);
					if (start< maxPos) {
						System.out.println("transcript "+tid+" not ordered (line "+lineCtr+").");
						return false;
					}
					lastTID= tid;
					maxPos= start;
