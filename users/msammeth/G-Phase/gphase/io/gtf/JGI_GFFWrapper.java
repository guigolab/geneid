...
public class JGI_GFFWrapper extends EncodeWrapper {
	...
	public static void main(String[] args) {
		...
		JGI_GFFWrapper myWrapper= new JGI_GFFWrapper(new File(fName).getAbsolutePath()); // testGTF.gtf
		try {
			myWrapper.read();
		} catch (Exception e) {
			e.printStackTrace(); 
	...
	/* (non-Javadoc)
			...
			public void read() throws Exception {
				...
				while (buffy.ready()) {
					...
					try {				
						...
						String tmp= toki.nextToken();
						while ((!tmp.equalsIgnoreCase("transcriptId"))&& (!tmp.equalsIgnoreCase("proteinId")))
							tmp= toki.nextToken();
						tmp= toki.nextToken();
						while(!Character.isDigit(tmp.charAt(tmp.length()- 1)))
							tmp= tmp.substring(0, tmp.length()- 1);
						newObj.addAttribute(GTFObject.TRANSCRIPT_ID_TAG, tmp);
						gtfVec.add(newObj);
					} catch (Exception e) {
			...
			public Transcript[] getTranscripts() {
				...
				for (int i = 0; i < keys.length; i++) {	// chromosomes
					...
					for (int j = 0; j < tkeys.length; j++) {	// transcripts
						...
						v.add(transcript);	// fill tHash with transcripts
					} 
				}
				
				return (Transcript[]) Arrays.toField(v);
			}

			public JGI_GFFWrapper(String absFName) {
				super(absFName);
			}
			...
			public static void main_Edu_batch(String[] args) {
				...
				for (int i = 0; i < fNames.length; i++) {
					...
					JGI_GFFWrapper myWrapper= new JGI_GFFWrapper(new File(fNames[i]).getAbsolutePath()); // testGTF.gtf
					try {
						myWrapper.read();
					} catch (Exception e) {
						e.printStackTrace(); 
