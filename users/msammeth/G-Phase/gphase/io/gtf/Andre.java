...
public class Andre extends EncodeWrapper {
	...
	public static void main(String[] args) {
		...
		Andre myWrapper= new Andre(new File(fName).getAbsolutePath()); // testGTF.gtf
		try {
			myWrapper.read();
		} catch (Exception e) {
			e.printStackTrace(); 
	...
	public void read() throws Exception {
		...
		gtfObj= (GTFObject[]) Arrays.toField(gtfVec);
			...
			public void read(String id) {
				...
				try {
					...
					while (buffy.ready()) {
						...
						GTFObject newObj= createGTFObject();
						try {				
							newObj.seqname= tokens[0];
							newObj.source= tokens[1];
							newObj.setFeature(tokens[2]);
							newObj.start= Integer.parseInt(tokens[3]);
							newObj.end= Integer.parseInt(tokens[4]);
							newObj.setScore(tokens[5]);
							newObj.setStrand(tokens[6]);
							newObj.setFrame(tokens[7]);
							
							String[] attr= tokens[8].split(";");
							for (int i = 0; i < attr.length; i++) {
								if (attr[i].indexOf(GTFObject.TRANSCRIPT_ID_TAG)>= 0) {
									String tid= attr[i].substring(attr[i].indexOf(' ')).trim();
									if (tid.startsWith("\""))
										tid.substring(1, tid.length()- 1);
									newObj.addAttribute(GTFObject.TRANSCRIPT_ID_TAG, tid);
								}
							}
						} catch (Exception e) {
							System.err.println("Invalid GTF format (line "+ lineCtr+"): "+e.toString());
			...
			public Gene assembleGene(Species spec) {
				
				HashMap hash= getGroups(GTFObject.TRANSCRIPT_ID_TAG, getGtfObj());	// cluster for genes?
				Gene ge= new Gene(spec, Gene.getUniqueID());
				Object[] o= hash.keySet().toArray();
				for (int i = 0; i < o.length; i++) {
					String tID= (String) o[i];
					Transcript trpt= new Transcript(tID);
					GTFObject[] gtfs= (GTFObject[]) Arrays.toField(hash.get(tID));	// gtf entries for 1 transcript
					GTFObject ff= (GTFObject) gtfs[0];
					trpt.setStrand(ff.getStrand());
					ge.setStrand(ff.getStrand());
					trpt.setGene(ge);					
					for (int k = 0; k < gtfs.length; k++) {		// exons 
						GTFObject f= (GTFObject) gtfs[k];
						if (f.isExon()) {
							Exon ex= new Exon(trpt, f.getExonID(), f.getStart(), f.getEnd());
							trpt.addExon(ex);
							trpt.updateBoundaries(ex);
						} else if (f.isCDS())
							trpt.addCDS(f.getStart(), f.getEnd());
					}
			...
			public static void main_Edu_batch(String[] args) {
				...
				for (int i = 0; i < fNames.length; i++) {
					...
					Andre myWrapper= new Andre(new File(fNames[i]).getAbsolutePath()); // testGTF.gtf
					try {
						myWrapper.read();
					} catch (Exception e) {
						e.printStackTrace(); 
