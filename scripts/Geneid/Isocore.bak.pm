...
sub new 
  {
      my ($class) = @_; 
      my $self = {
		  _boundaries_of_isochore => undef,
		  _Absolute_cutoff_exons => undef,
		  _Coding_cutoff_oligos => undef,
		  _Site_factor => undef,
		  _Exon_factor => undef,
		  _HSP_factor => undef,
...
sub Markov_oligo_logs_file {
    my ( $self, $val ) = @_;
    $self->{_Markov_oligo_logs_file} = $val if defined($val);
    return $self->{_Markov_oligo_logs_file};
}
