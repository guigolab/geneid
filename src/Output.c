...
/*  $Id: Output.c,v 1.3 2000-08-08 14:18:29 eblanco Exp $  */
  ...
  if (EXP) 
    {
      ...
      PrintExons(exons, nExons, FIRST + INTERNAL + TERMINAL + SINGLE, 
		 Locus, l1, l2, Sequence, dAA);
    }
}

/* Print genes using selected format */
void OutputGene(packGenes* pg, long nExons, char* Locus,
                char* Sequence, gparam* gp, dict* dAA)
{
  /* Recover the Best Gene recursively */
  if (nExons>0)
    {
