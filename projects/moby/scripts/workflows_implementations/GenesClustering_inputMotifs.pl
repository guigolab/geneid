...
my $matrices_input_xml = [];

if (! -f $database) {
    print STDERR "Error, can't find any file called $database!\n";
    exit 1;
}
else {
    # Get an array of matrices in XML format

    my $matrices_str = qx/cat $database/;
    my @matrices = split ('</MatrixFloat>', $matrices_str);
    foreach my $matrix (@matrices) {
	
	# The last element is rubbish, leave it aside !
	...
	$matrix .= "</MatrixFloat>\n";
	push (@$matrices_input_xml, $matrix);
    }
}
...
$moby_response = $Service->execute (XMLinputlist => [
						     ["$articleName_1", $input_xml, "$articleName_2", $matrices_input_xml, "threshold", $threshold_xml]
						     ]);
