...
Usage:
    ...
    GenesClustering_LIST.pl [-h] -x {Moby Central} -f {list of gene identifiers file} -s {Species of the genes} -v {upstream sequence length} -w {downstream sequence length} -t {MatScan threshold} -d {MatScan database} -a {meta-alignment alpha penalty} -l {meta-alignment lambda penalty} -u {meta-alignment mu penalty} -g {multiple meta-alignment gamma penalty} -r {non-collinearity penalty} -m {Hierarchical clustering method} -e {sota distance function} -p {sota resource threshold} -o {Output directory} -c {configuration file}
	-h help
...
Examples using some combinations:
	perl GenesClustering_LIST.pl -x 2 -f /home/ug/arnau/data/breast_pregnancy/both.lst -c \$HOME/.workflow.config -s homo_sapiens -v 1000 -w 500 -t 0.80 -d jaspar -a 0.5 -l 0.1 -u 0.1 -g -10 -r 100 -m nearest -e euclidean -p 20 -o output
...
BEGIN {
    ...
    use vars qw/$opt_h $opt_x $opt_f $opt_c $opt_s $opt_v $opt_w $opt_t $opt_d $opt_a $opt_l $opt_u $opt_m $opt_g $opt_r $opt_e $opt_p $opt_o $opt_z $output_dir $gmaster_home/;
    
    # $gmaster_home = $ENV{'HOME'};
    $gmaster_home = "/home/ug/gmaster";
    
    # these are switches taking an argument (a value)
    my $switches = 'x:zhf:c:s:v:w:t:d:a:l:u:m:g:r:e:p:o:';
