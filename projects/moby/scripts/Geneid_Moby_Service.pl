...
my $profile    = "Homo sapiens (suitable for mammals)";
my $strands    = "Both";
my @exons      = ();
...
#						    ["$articleName", $input, 'profile', $profile_xml, 'strand', $strand_xml, 'signals', $signal_start_codons_xml, 'signals', $signal_stop_codons_xml]
#						   ]);

# with some reported exons options

    my $result = $Service->execute(XMLinputlist => [
						    ["$articleName", $input, 'profile', $profile_xml, 'strand', $strand_xml, 'signals', $signals_xml, 'exons', $exons_xml]
						   ]);

# my $result = $Service->execute(XMLinputlist => [
# 						["$articleName", $input, 'profile', $profile_xml, 'strand', $strand_xml]
# 						]);
