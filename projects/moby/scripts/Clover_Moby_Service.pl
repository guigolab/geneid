...
sub help {
return <<"END_HELP";
Description: Execute Clover Moby services available from genome.imim.es
Usage:

    Clover_Moby_Service.pl [-h] -x {Moby Central} -s {Service Name} -f {sequence FASTA file} -m {motif matrices file}
	-h help
...
Examples using some combinations:
	perl Clover_Moby_Service.pl -x 2 -s runCloverGFFCollection -f /home/ug/arnau/data/promoterExtraction/ENSG00000197785.upstream_region.5000.fa -m /home/ug/arnau/data/MeMe/meme2matrix.xml
...
if ($serviceName eq "runCloverGFFCollectionVsInputMatrices") {
...
# Pvalue_Threshold parameter

my $pvalue_threshold_xml = <<PRT;
<Value>$pvalue_threshold</Value>
PRT
...
if ($serviceType eq "Collection") {
    ...
    if ($serviceName eq "runCloverGFFCollection") {
	...
	$result = $Service->execute(XMLinputlist => [
						     ["$sequence_articleName", $inputs, "motif_weight_matrices", [$input_matrices_xml], 'pvalue_threshold', $pvalue_threshold_xml, 'matrix mode', $matrix_mode_xml]
						     ]);
    }
}
else {
    ...
    my $input = $inputs->[0];
    $result = $Service->execute(XMLinputlist => [
						 ["$sequence_articleName", $input, 'p_value threshold', $pvalue_threshold_xml, 'motif database' => $matrix_xml, 'matrix mode' => $matrix_mode_xml]
						 ]);
}
