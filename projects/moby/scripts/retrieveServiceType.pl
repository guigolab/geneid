...
sub help {
return <<"END_HELP";
Description: Retrieve a service type with its relationships from Moby Central
Usage:

retrieveServiceType.pl [-h] -x {Moby Central} -t {serviceType Name}
	-h help
...
Examples using some combinations:
	perl retrieveServiceType.pl -x Inab -t serviceType
...
BEGIN {
	...
	if (defined($opt_h) || !defined($opt_x) || !defined($opt_t)){
		print help;
		exit 0;
	}
...
my $serviceTypeName = $opt_t;

# Retrieve the definition (type, description, relations, ...) from the input object.
my $types = $MOBYCentral->retrieveServiceTypes();

print "\n";

if (! defined $types) {
    print STDERR "Error, no defined service types!\n";
}

if (my $desc = $types->{$serviceTypeName}) {
    print "definition for service type, $serviceTypeName, $desc\n";
}
else {
    print "$serviceTypeName has no description!\n";
}

my $serviceTypeRelationships = $MOBYCentral->Relationships('serviceType' => $serviceTypeName);

print STDERR "Dumping $serviceTypeRelationships, " . Dumper ($serviceTypeRelationships) . "\n";
