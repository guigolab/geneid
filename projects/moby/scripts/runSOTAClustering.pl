...
sub help {
return <<"END_HELP";
Description: Execute runSOTAClustering MOBY service
Usage:

    runSOTAClustering.pl [-h] -x {Moby Central} -f {score matrix file}
	-h help
...
if ($centre eq "imim") {
  ...
  $articleName = "gene_score_matrix";
  $::authURI   = 'genome.imim.es';
}


my $score_matrix_file = "/home/ug/arnau/Moby_misc/output_leptins_transfac_0.92/score_matrix.txt";

if (defined $opt_f) {
  $score_matrix_file = $opt_f;
}
...
my $score_matrix = qx/cat $score_matrix_file/;

my $score_matrix_xml = <<PRT;
<MicroArrayData_Text namespace="" id="">
  <String namespace="" id="" articleName="content"><![CDATA[$score_matrix]]></String>
</MicroArrayData_Text>
PRT

# Parameters

# ['euclidean', 'square', 'correlation', 'offset', 'spearman', 'jackknife']
my $distance = "euclidean";
my $distance_xml = "<Value>euclidean</Value>";
...
my $result = $Service->execute(XMLinputlist => [
						["$articleName", $score_matrix_xml, "distance", $distance_xml]
					       ]);
