...
Description: Register a service in Moby Central
  Usage:
	...
	perl registerService.pl -x 2 -s runMultiMetaAlignment
...
my ($REG) = $Central->registerService(
				      ...
				      description  => "Runs Multiple-Meta-Alignment software to perform multiple non-collinear transcription factor map alignments of promoter regions. It returns the multiple-meta-alignment output in meta-alignment text format.",
				      category     => "moby",
				      URL          => $::URL,
				      input		=> [
							    ['maps', [['GFF' => \@namespaces]]] # collection of one object type
							   ],
				      output		=> [
							    ['multi_meta_predictions', ['Meta_Alignment_Text' => \@namespaces]] # simple
							   ],
				      secondary	=> {
					  'alpha penalty' => {
					      datatype => 'Float',
