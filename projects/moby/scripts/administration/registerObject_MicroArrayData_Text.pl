...
my $objectName = "MicroArrayData_Text";
my $objectDescription = "Micro-array data in text format representing gene expression patterns. Each line must have the gene name and the expression values and the expression values for each condition. All the data items must be separated by tabulators";
