...
Description: Register a service in Moby Central
  Usage:
	...
	perl registerService.pl -x 2 -s runRepeatMaskerCollection
...
my $serviceName = $opt_s || "runRepeatMaskerCollection";
...
my ($REG) = $Central->registerService(
				      ...
				      description  => "Runs RepeatMasker to mask repeat sequences a set of DNA sequences",
				      category     => "moby",
				      URL          => $::URL,
				      input		=> [
