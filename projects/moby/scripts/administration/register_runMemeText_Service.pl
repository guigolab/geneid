...
Description: Register a service in Moby Central
  Usage:
	...
	perl registerService.pl -x 2 -s runMemeText
...
my $serviceName = $opt_s || "runMemeText";
...
my ($REG) = $Central->registerService(
				      ...
				      description  => "Analyzes a set of protein or DNA sequences for similarities among them and produces a description (motif) for each pattern it discovers. The results are returned in MEME text format",
				      category     => "moby",
				      URL          => $::URL,
				      input		=> [
