...
my ($REG) = $Central->registerService(
				      ...
				      description  => "Promoter regions analysis program. Meta-alignment produces alignments of sequences of TF binding sites. It returns the predictions in 'Meta-alignment' format. Use runMatScanGFF to produce the input GFF files",
				      category     => "moby",
				      URL          => $::URL,
				      input		=> [
							    ['map1', ['GFF' => \@namespaces]],
