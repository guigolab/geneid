...
Description: Register a service in Moby Central
  Usage:
	...
	perl registerService.pl -x 2 -s runGFF2JPEG
...
my $serviceName = $opt_s || "runGFF2JPEG";
my $serviceType = "Displaying";
my @namespaces  = ();
...
my ($REG) = $Central->registerService(
				      ...
				      description  => "Runs gff2ps software to vizualize a set of GFF annotations maps. It returns an encoded image in JPEG format.",
				      category     => "moby",
				      URL          => $::URL,
				      input		=> [
				      ...
				      output		=> [
							    ['image', ['b64_encoded_jpeg' => \@namespaces]] # simple
							   ],
				      );
