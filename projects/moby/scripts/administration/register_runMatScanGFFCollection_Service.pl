...
Description: Register a service in Moby Central
  Usage:
	...
	perl registerService.pl -x 2 -s runMatScanGFFCollection
...
my $serviceName = $opt_s || "runMatScanGFFCollection";
...
my ($REG) = $Central->registerService(
				      ...
				      description  => "Analyzes a collection of DNA sequences for putative transcription factor binding sites from Transfac or Jaspar and reports them in GFF format",
				      category     => "moby",
				      URL          => $::URL,
				      input		=> [
