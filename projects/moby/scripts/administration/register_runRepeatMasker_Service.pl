...
Description: Register a service in Moby Central
  Usage:
	...
	perl registerService.pl -x 2 -s runRepeatMasker
...
my $serviceName = $opt_s || "runRepeatMasker";
my $serviceType = "DNA_Repeat_Masking";
my @namespaces  = ();
...
my ($REG) = $Central->registerService(
				      ...
				      description  => "Run RepeatMasker to mask repeat sequences within a DNA sequence",
				      category     => "moby",
				      URL          => $::URL,
				      input		=> [
				      ...
				      output		=> [
							    ['masked_sequence', ['DNASequence' => \@namespaces]],
							   ],
				      secondary	=> {
					  'species' => {
					      datatype => 'String',
