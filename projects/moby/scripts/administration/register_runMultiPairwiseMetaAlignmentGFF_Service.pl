...
Description: Register a service in Moby Central
  Usage:
	...
	perl registerService.pl -x 2 -s runMultiPairwiseMetaAlignmentGFF
...
my $serviceName = $opt_s || "runMultiPairwiseMetaAlignmentGFF";
...
my ($REG) = $Central->registerService(
				      ...
				      description  => "Runs Meta-alignment software on a multiple running mode, receiving a collection of maps, making pairs of them and, foreach pair, producing, in GFF format, alignments of sequences of TF binding sites",
				      category     => "moby",
				      URL          => $::URL,
				      input		=> [
							    ['maps', [['GFF' => \@namespaces]]],
