...
Description: Register a service in Moby Central
  Usage:
	...
	perl registerService.pl -x 2 -s runPhrapWithQualityData
...
my $serviceName = $opt_s || "runPhrapWithQualityData";
my $serviceType = "Sequence_Assembly";
my @namespaces  = ();
...
my ($REG) = $Central->registerService(
				      ...
				      description  => "Runs Phrap to Assemble a set of DNA sequences, taking into account base quality data",
				      category     => "moby",
				      URL          => $::URL,
				      input	   => [
