...
Description: Deregister an object in Moby Central
  Usage:
	...
	perl deregisterObject.pl -x 1 -o GFF4 -w genome.imim.es
...
BEGIN {
    ...
    use vars qw/$opt_h $opt_x $opt_o $opt_w/;
    ...
    if (defined($opt_h) || !defined($opt_x) || !defined($opt_o) || !defined($opt_w)){
	print STDERR help;
	exit 0;
    }
}
...
if ($REG->success) {
    ...
    # The result is valid.
    print "The '$objectName' object has been deregistered in $opt_x successfully: ", $REG->success, "\n";
