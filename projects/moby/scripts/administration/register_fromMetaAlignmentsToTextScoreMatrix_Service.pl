...
Description: Register a service in Moby Central
  Usage:
	...
	perl registerService.pl -x 2 -s fromMetaAlignmentsToTextScoreMatrix
...
my $serviceName = $opt_s || "fromMetaAlignmentsToTextScoreMatrix";
...
my ($REG) = $Central->registerService(
				      ...
				      description  => "Parses a collection of meta-alignment outputs to produce a text-formatted sequence similarity score matrix",
				      category     => "moby",
				      URL          => $::URL,
				      input		=> [
				      ...
				      output		=> [
							    ['microarraydata', ['MicroArrayData_Text' => \@namespaces]]
							   ],
				      );
