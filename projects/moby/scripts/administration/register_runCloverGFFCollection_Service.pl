...
Description: Register a service in Moby Central
  Usage:
	...
	perl registerService.pl -x 2 -s runCloverGFFCollection
...
my $serviceName = $opt_s || "runCloverGFFCollection";
...
my ($REG) = $Central->registerService(
				      ...
				      description  => "Uses Clover to analyze a collection of DNA sequences for putative transcription factor binding sites from Transfac or Jaspar and reports them in GFF format",
				      category     => "moby",
				      URL          => $::URL,
				      input		=> [
				      ...
				      output		=> [
							    ['clover_predictions', [['GFF' => \@namespaces]]] # collection of one object type
							   ],
				      secondary	=> {
					 'motif database' => {
					      datatype => 'String',
					  ...
					  'motif database filtering' => {
					      datatype => 'String',
					      enum     => ['All','Vertebrates Only', 'Invertebrates Only'],
					      default  => 'All',
					  },
					  'matrix mode' => {
					      datatype => 'String',
					  ...
					  'background sequences' => {
					      datatype => 'String',
					      enum     => ['None','Human','Mouse', 'Rat', 'Drosophila'],
					      default  => 'None'
					  },
				      }
				      );
