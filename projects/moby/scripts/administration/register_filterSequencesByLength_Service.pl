...
Description: Register a service in Moby Central
  Usage:
	...
	perl registerService.pl -x 2 -s filterSequencesByLength
...
my $serviceName = $opt_s || "filterSequencesByLength";
...
my ($REG) = $Central->registerService(
				      ...
				      description  => "Filters a set of sequences based on their length. Only sequences which length is greater than the length cutoff will be returned.",
				      category     => "moby",
				      URL          => $::URL,
				      input	   => [
