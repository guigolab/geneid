...
Description: Register a service in Moby Central
  Usage:
	...
	perl registerService.pl -x 2 -s runDustCollection
...
my $serviceName = $opt_s || "runDustCollection";
...
my ($REG) = $Central->registerService(
				      ...
				      description  => "Runs dust to Mask low-complexity regions within a set of DNA sequences",
				      category     => "moby",
				      URL          => $::URL,
				      input		=> [
