...
Description: Register a service in Moby Central
  Usage:
	...
	perl registerService.pl -x 2 -s runDustFASTA_NA_multi
...
my $serviceName = $opt_s || "runDustFASTA_NA_multi";
my $serviceType = "DNA_Low_Complexity_Masking";
my @namespaces  = ();
...
my ($REG) = $Central->registerService(
				      ...
				      description  => "Runs dust to mask low-complexity regions within a set of DNA sequences in FASTA format",
				      category     => "moby",
				      URL          => $::URL,
				      input		=> [
				      ...
				      output		=> [
							    ['masked_sequences', ['FASTA_NA_multi' => \@namespaces]],
							   ],
				      );
