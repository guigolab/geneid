...
Description: Register a service in Moby Central
  Usage:
	...
	perl registerService.pl -x 2 -s filterSequencesAndQualityDataByLength
...
my $serviceName = $opt_s || "filterSequencesAndQualityDataByLength";
my $serviceType = "Sequence_Filtering";
my @namespaces  = ();
...
my ($REG) = $Central->registerService(
				      ...
				      description  => "Filters a set of sequences based on their length. Only sequences which length is greater than the length cutoff will be returned. It also filters out the quality data associated with the removed sequences.",
				      category     => "moby",
				      URL          => $::URL,
				      input	   => [
