...
my ($REG) = $Central->registerService(
				      ...
				      description  => "Promoter regions analysis program. runMultiMetaAlignment runs Meta-alignment on a multiple mode, receiving a collection of maps, making pairs of them and, foreach pair, it produces alignments of sequences of TF binding sites. It returns the predictions in GFF format. Use runMatScanGFF to produce the input GFF files",
				      category     => "moby",
				      URL          => $::URL,
				      input		=> [
							    ['maps', [['GFF' => \@namespaces]]],
