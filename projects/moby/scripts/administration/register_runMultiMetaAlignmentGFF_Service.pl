...
my ($REG) = $Central->registerService(
				      ...
				      description  => "Runs Multiple-Meta-Alignment software to perform multiple non-collinear transcription factor map alignments of promoter regions. It returns the multiple-meta-alignment output in GFF format.",
				      category     => "moby",
				      URL          => $::URL,
				      input		=> [
				      ...
				      output		=> [
							    ['multi_meta_predictions', ['GFF' => \@namespaces]] # simple
							   ],
				      secondary	=> {
					  'alpha penalty' => {
					      datatype => 'Float',
