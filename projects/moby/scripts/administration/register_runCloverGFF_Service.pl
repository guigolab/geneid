...
Description: Register a service in Moby Central
  Usage:
	...
	perl registerService.pl -x 2 -s runCloverGFF
...
my $serviceName = $opt_s || "runCloverGFF";
...
my ($REG) = $Central->registerService(
				      ...
				      description  => "Uses Clover to analyze a DNA sequence for putative transcription factor binding sites from Transfac or Jaspar and reports them in GFF format",
				      category     => "moby",
				      URL          => $::URL,
				      input		=> [
				      ...
				      output		=> [
							    ['clover_predictions', ['GFF' => \@namespaces]]
							   ],
				      secondary	=> {
					  'motif database' => {
					      datatype => 'String',
