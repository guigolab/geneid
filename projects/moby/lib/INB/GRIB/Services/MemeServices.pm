...
sub _do_query_Meme {
    ...
    my ($report) = MEME_call (sequences  => \%sequences, format => $_output_format, debug => $_debug, parameters => \%parameters);
...
=head2 runMemeText
 ...
 Usage   : Esta función está pensada para llamarla desde un cliente SOAP. 
         ...
         :   </moby:mobyContent>
         :  </moby:MOBY>
 Returns : Return a text_formatted object with MEME results
