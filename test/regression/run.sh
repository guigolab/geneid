...
CASES=(
  ...
  # morc_rc_prot: the '-' strand UTR gene with -A (protein output). Guards the
  # TranslateGene fix -- a leading 5' UTR feature used to defeat the first-CDS-
  # exon guard, leaving rmdProt uninitialised and prepending stack garbage to the
  # protein. The golden protein must start cleanly at M (== the forward MORC protein).
  "morc_rc_prot|param/human.rnaseq.param|-3U -u -A -R samples/morc_rc.introns.gff -S samples/morc_rc.plus.bw,samples/morc_rc.minus.bw|samples/morc_rc.fa"
)
