...
| `rnaseq_bw_nou` | human.rnaseq | chr21 MORC3 locus | bigWig `-S` coverage **without** `-u`: guards the `-u`-decoupling — coverage scores exons via `sr[]` with no UTR prediction and no `readcount[]` allocation (must run and predict CDS, emit no UTR) |
