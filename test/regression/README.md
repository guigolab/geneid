...
| `morc_rc_rnaseq` | human.rnaseq | reverse-complemented MORC3 window | The `rnaseq` case on the **minus strand**: reversed intron (`-R`) + stranded coverage (`-S`) evidence drive the mirror 17-CDS/2-UTR gene on `-`, exercising the reverse-strand coordinate paths (`sr[]` genomic↔RSequence flip). Fixtures regenerated by `make_morc_rc.py` |
| `morc_rc_prot` | human.rnaseq | reverse-complemented MORC3 window | The `-` strand UTR gene with `-A` (protein output). Guards the `TranslateGene` fix: a leading 5′ UTR feature used to defeat the first-CDS-exon guard, prepending uninitialised `rmdProt` stack garbage to the protein. The golden protein must start cleanly at `M` |
